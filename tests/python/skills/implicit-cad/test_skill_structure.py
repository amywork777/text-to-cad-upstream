...
class ImplicitCadSkillStructureTests(unittest.TestCase):
    ...
    def test_gen_package_is_runnable_as_a_module(self) -> None:
        # The parser moved into cadgen.cli when skills stopped carrying their own; the
        # skill keeps a runnable `scripts/gen` package, which is what callers depend on.
        gen_dir = self.skill_root / "scripts" / "gen"
        self.assertTrue((gen_dir / "__main__.py").is_file())
        self.assertIn("cadgen.cli", (gen_dir / "__main__.py").read_text(encoding="utf-8"))
