"""``cadgen 3mf snapshot`` — render a 3MF mesh.
...
The mesh half of ``cadgen step snapshot``, re-homed: 3MF is a format with a door
(``cadgen 3mf build``), so its snapshot belongs behind the same door rather than
...
def main(argv: Sequence[str] | None = None, *, prog: str = "cadgen 3mf snapshot") -> int:
