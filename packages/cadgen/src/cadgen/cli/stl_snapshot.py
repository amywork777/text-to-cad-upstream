...
The mesh half of ``cadgen step snapshot``, re-homed: STL is a format with a
door (``cadgen stl build``), so its snapshot belongs behind the same door. Its
...
DEFAULT_PROG = "cadgen stl snapshot"
VERB = ("cadgen.stl", "snapshot")
