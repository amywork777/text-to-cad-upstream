...
// delta T_target * inverse(T_athletic) in the fixed model frame (mm, +X
// forward, +Y robot-left, +Z up, pelvis waist-yaw center at the origin).
//
// Animation is deliberately ignorant of the model's typed mates: it
// re-describes the motion in its own FK, so editing a clip can never
// invalidate a build. Seven clips — four in-place gaits (walk, stride, run,
// jump) and three showpieces (Elvis dance, handstand, karate kick) — each a
// pure function of t.
//
...
// Athletic ready stance angles (deg), mirrored from lib/chain.py.
const ATHLETIC = {
  HIP_PITCH: -16,
  KNEE: 32,
  ANKLE_PITCH: -16,
...
// Kinematic chain in root-to-leaf order, mirrored from lib/chain.py.
...
// Each link is a labeled GROUP compound, so it is targeted by occurrence id
// (a label would only match a rendered part) and covers its whole subtree.
const LINK_REFS = {
  pelvis: "#o1.1",
  torso: "#o1.2",
  neck_collar: "#o1.3",
  head: "#o1.4",
  hip_bracket_left: "#o1.5",
  hip_carrier_left: "#o1.6",
  thigh_left: "#o1.7",
  shin_left: "#o1.8",
  ankle_link_left: "#o1.9",
  foot_left: "#o1.10",
  shoulder_pod_left: "#o1.11",
  yaw_housing_left: "#o1.12",
  bicep_left: "#o1.13",
  forearm_left: "#o1.14",
  wrist_carrier_left: "#o1.15",
  hand_left: "#o1.16",
  hip_bracket_right: "#o1.17",
  hip_carrier_right: "#o1.18",
  thigh_right: "#o1.19",
  shin_right: "#o1.20",
  ankle_link_right: "#o1.21",
  foot_right: "#o1.22",
  shoulder_pod_right: "#o1.23",
  yaw_housing_right: "#o1.24",
  bicep_right: "#o1.25",
  forearm_right: "#o1.26",
  wrist_carrier_right: "#o1.27",
  hand_right: "#o1.28"
};

// The rigid delta taking a link's baked athletic placement onto its target
// placement, as (axis, degrees about the model origin) + (translation):
// Rd = R_t R_o^T, t = p_t - Rd p_o. Handle calls PREMULTIPLY, so rotating
// first and translating second composes to T * Rd — exactly that delta.
function deltaAxisAngle(original, target) {
  ...
  // atan2 of the antisymmetric part against the trace, NOT acos(trace): the
  // two share the same rounding, so a near-zero rotation stays accurate
  // instead of picking up the sqrt-shaped error acos has at its endpoints.
  const skew = [Rd[2][1] - Rd[1][2], Rd[0][2] - Rd[2][0], Rd[1][0] - Rd[0][1]];
  const twoSin = Math.hypot(skew[0], skew[1], skew[2]);
  const twoCos = (Rd[0][0] + Rd[1][1] + Rd[2][2]) - 1;
  const angle = Math.atan2(twoSin, twoCos);
  let axis = [0, 0, 1];
  if (angle > 1e-12) {
    if (twoSin > 1e-6) {
      axis = skew.map((v) => v / twoSin);
    } else {
      // Near a half turn: sin(angle) vanishes, so read the axis off the
      // symmetric part (Rd + I = 2 a a^T) from its largest diagonal.
      const diag = [Rd[0][0] + 1, Rd[1][1] + 1, Rd[2][2] + 1];
      let k = 0;
      if (diag[1] > diag[k]) k = 1;
      if (diag[2] > diag[k]) k = 2;
      const scale = Math.sqrt(Math.max(diag[k], 1e-12)) * 2;
      const column = [Rd[0][k] + (k === 0 ? 1 : 0), Rd[1][k] + (k === 1 ? 1 : 0), Rd[2][k] + (k === 2 ? 1 : 0)];
      axis = column.map((v) => v / scale);
      const norm = Math.hypot(axis[0], axis[1], axis[2]) || 1;
      axis = axis.map((v) => v / norm);
    }
  }
  return { axis, degrees: (angle * 180) / Math.PI, t };
}

function applyFrames(m, targetFrames) {
  for (const [link, ref] of Object.entries(LINK_REFS)) {
    const delta = deltaAxisAngle(ATHLETIC_FRAMES[link], targetFrames[link]);
    const handle = m.get(ref);
    if (delta.degrees > 1e-9) {
      handle.rotate(delta.axis, delta.degrees, [0, 0, 0]);
    }
    if (Math.abs(delta.t[0]) + Math.abs(delta.t[1]) + Math.abs(delta.t[2]) > 1e-9) {
      handle.translate(delta.t);
    }
  }
}

// The retired sidecar exposed lift/swing/sway/stride as live viewer
// parameters; clips are pure functions of t, so the published defaults are
// baked in here.
const SCALES = { legLift: 1, armSwing: 1, torsoSway: 1, strideLength: DEFAULT_STRIDE_MM };

function framesForGait(gait, phase) {
  if (gait === "stride") {
    return fkFrames(
  ...
  if (gait === "kick" || gait === "handstand" || gait === "dance") {
    const pose = gait === "kick"
      ? kickPose(phase)
      : gait === "handstand"
        ? handstandPose(phase)
        : dancePose(phase);
    return fkFrames(pose.angles, pose.extras, pose.rootOffset, pose.rootPitchDeg);
  }
  return fkFrames(
...
function loopClip(label, description, gait, duration) {
  return {
    label,
    description,
    duration,
    loop: true,
    update(t, m) {
      applyFrames(m, framesForGait(gait, wrap01(t / duration)));
    }
  };
}
...
export const clips = {
  danceLoop: loopClip(
    "Elvis dance",
    "The King: weight on the right leg, left leg kicked out to the left on a shaking pointed toe, right finger pointing up to the right with a beat pulse, hip swivel and chin up.",
    "dance",
    3.2
  ),
  handstandLoop: loopClip(
    "Handstand",
    "Toe-pivot fold, press to a palms-flat inverted hold with a breathing wobble, and back up to the ready stance.",
    "handstand",
    4.6
  ),
  kickLoop: loopClip(
    "Karate kick",
    "Weight shifts over the left leg, the right leg chambers and snaps a ball-of-foot front kick at hip height behind a fists-up guard.",
    "kick",
    2.8
  ),
  jumpLoop: loopClip(
    "Jump in place",
    "Crouch, leap with hands overhead, and land with a springy knee-absorbed wobble before settling back to the ready stance.",
    "jump",
    1.8
  ),
  runLoop: loopClip(
    "Run in place",
    "Running cadence with flight phases: body bounce, toe-pivot push-off, high heel recovery, forward lean, and pumping bent arms.",
    "run",
    0.8
  ),
  strideLoop: loopClip(
    "Stride in place",
    "Bigger strides: legs sweep back and forward with the stance foot sliding flat on the ground, wider arm swing, stronger torso counter-sway.",
    "stride",
    1.9
  ),
  walkLoop: loopClip(
    "Walk in place",
    "Continuous march loop: alternating leg lift with planted stance feet, antiphase arm swing, torso counter-sway, and a head that keeps facing forward.",
    "march",
    1.5
  )
};
