...
def wheel_lattice() -> bd.Compound:
    ...
    parts: list[bd.Shape] = [
        ...
        style(bd.Torus(65.0, 1.8, rotation=(90, 0, 0)).moved(bd.Location((0.0, -21.4, 80.0))), "wheel_outer_bead_visible", "dark_graphite"),
        style(bd.Torus(65.0, 1.8, rotation=(90, 0, 0)).moved(bd.Location((0.0, 21.4, 80.0))), "wheel_outer_bead_inner", "dark_graphite"),
    ]
...
def tread_elements() -> bd.Compound:
    parts: list[bd.Shape] = []
    rim_radius = 65.0
    tread_height = 5.0
...
def brake_disc() -> bd.Shape:
    disc = bd.Cylinder(41.0, 5.0, rotation=(90, 0, 0)).moved(bd.Location((0.0, 29.0, 80.0)))
    for index in range(24):
        ...
        disc = disc - bd.Cylinder(2.2, 8.0, rotation=(90, 0, 0)).moved(bd.Location(hole_center))
    disc = softened(disc, 0.4)
    return style(disc, "brake_disc_82mm_vented", "brushed_steel")


def brake_caliper() -> bd.Compound:
...
def bushing(label: str, center: Vector3, length: float = 15.0) -> bd.Shape:
    return style(softened(cylinder_y(label, center, 6.2, length, "satin_titanium"), 0.5), label, "satin_titanium")


def control_arms() -> tuple[bd.Compound, bd.Compound]:
...
def spring_along_axis(label: str, start: Vector3, end: Vector3) -> bd.Compound:
...
def coilover_shock() -> bd.Compound:
    top = (-90.0, -8.0, 118.0)
...
def triangular_cutout_tool(label: str, x_center: float, vertices_yz: Sequence[tuple[float, float]]) -> bd.Shape:
    plane = bd.Plane(origin=(x_center, 0.0, 0.0), x_dir=(0.0, 1.0, 0.0), z_dir=(1.0, 0.0, 0.0))
    with bd.BuildPart() as tool:
        with bd.BuildSketch(plane):
            bd.Polygon(*vertices_yz)
        bd.extrude(amount=13.0, both=True)
    tool.part.label = label
    return tool.part


def chassis_bulkhead() -> tuple[bd.Shape, bd.Compound]:
    plate = bd.Box(8.0, 70.0, 95.0).moved(bd.Location((-95.0, 20.0, 80.0)))
    cutouts = [
...
def countersunk_display_base() -> bd.Shape:
    with bd.BuildPart() as part:
        with bd.BuildSketch():
            bd.RectangleRounded(170.0, 110.0, 6.0)
        bd.extrude(amount=8.0)
    base = part.part.moved(bd.Location((BASE_CENTER_X, 0.0, 0.0)))
    hole_positions = [
    ...
    for x, y in hole_positions:
        base = base - bd.Cylinder(3.2, 12.0).moved(bd.Location((x, y, 4.0)))
        base = base - bd.Cone(3.2, 6.4, 5.0).moved(bd.Location((x, y, 6.3)))
    base = softened(base, 1.1)
    return style(base, "display_base_rounded_countersunk", "brushed_aluminum")


def fasteners() -> bd.Compound:
    parts: list[bd.Shape] = []
...
@step(out="../STEP/lunar_rover_corner_assembly.step")
def lunar_rover_corner_assembly():
