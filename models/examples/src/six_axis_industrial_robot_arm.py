...
def _make_axis_cylinder(
    ...
    part = cyl.part.moved(bd.Location(center, _axis_rotation(axis)))
...
def _make_axis_ring(
    ...
    part = ring.part.moved(bd.Location(center, _axis_rotation(axis)))
    return _set_metadata(part, label, color)
...
def _make_screw_head(
    ...
    with bd.BuildPart() as screw:
        ...
        with bd.Locations(bd.Location((0.0, 0.0, height / 2.0 - 0.18))):
            bd.Cylinder(
                radius=radius * 0.43,
                height=0.45,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER),
    ...
    part = screw.part.moved(bd.Location(center, _axis_rotation(axis)))
    return _set_metadata(part, label, color)
...
def _make_rounded_box(
    ...
    part = box.part.moved(bd.Location(center, rotation))
    return _set_metadata(part, label, color)
...
def _make_display_base():
    with bd.BuildPart() as base:
        ...
        for x_abs in (-20.0, 120.0):
            for y_pos in (-50.0, 50.0):
                ...
                with bd.Locations(bd.Location((x_pos, y_pos, DISPLAY_BASE_THICKNESS - 3.2))):
                    bd.Cone(
                        bottom_radius=3.0,
                        top_radius=6.0,
                        height=3.4,
    ...
    part = base.part.moved(bd.Location(DISPLAY_BASE_CENTER))
...
def _make_pedestal():
    with bd.BuildPart() as pedestal:
        with bd.BuildSketch(bd.Plane.XY):
            bd.RectangleRounded(46.0, 58.0, 7.0, align=(bd.Align.CENTER, bd.Align.CENTER))
        bd.extrude(amount=43.0)
        _safe_fillet(pedestal.edges(), 2.5)
    part = pedestal.part.moved(bd.Location((0.0, 0.0, 47.0)))
...
def _make_shoulder_lug(label: str, y_center: float):
    with bd.BuildPart() as lug:
        with bd.Locations(bd.Location((3.0, y_center, 73.5))):
            bd.Box(34.0, 12.0, 37.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        with bd.Locations(bd.Location((0.0, y_center, 90.0))):
            bd.Cylinder(
                radius=22.0,
                height=12.0,
...
def _make_upper_arm_link(label: str, y_center: float):
    ...
    with bd.BuildPart() as link:
        with bd.BuildSketch(bd.Plane.XZ.offset(-5.0)):
            bd.RectangleRounded(link_length, 18.0, 8.0, align=(bd.Align.CENTER, bd.Align.CENTER))
        bd.extrude(amount=10.0)

        with bd.BuildSketch(bd.Plane.XZ.offset(-6.2)):
            bd.RectangleRounded(56.0, 7.4, 3.5, align=(bd.Align.CENTER, bd.Align.CENTER))
        bd.extrude(amount=12.4, mode=bd.Mode.SUBTRACT)
        _safe_fillet(link.edges(), 1.5)

    part = link.part.moved(bd.Location(mid, (0.0, link_angle, 0.0)))
...
def _make_forearm_housing():
    ...
    part = forearm.part.moved(bd.Location(center, (0.0, angle, 0.0)))
...
def _make_forearm_panel(label: str, side: float):
    ...
    with bd.BuildPart() as panel:
        with bd.Locations(bd.Location((6.0, side * 15.3, 0.0))):
            bd.Box(50.0, 1.2, 10.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        _safe_fillet(panel.edges(), 0.7)

    part = panel.part.moved(bd.Location(center, (0.0, angle, 0.0)))
...
def _make_wrist_yoke_lug(label: str, y_center: float):
    with bd.BuildPart() as lug:
        with bd.Locations(bd.Location((164.5, y_center, 128.0))):
            bd.Box(25.0, 7.0, 17.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        with bd.Locations(bd.Location((174.0, y_center, 128.0))):
            bd.Cylinder(
                radius=14.5,
                height=7.0,
                rotation=(90.0, 0.0, 0.0),
...
def _make_tool_flange():
    with bd.BuildPart() as flange:
        ...
        for index in range(6):
            ...
            with bd.Locations(bd.Location((0.0, y_pos, z_pos))):
                bd.Cylinder(
                    radius=2.0,
                    height=9.0,
                    rotation=(0.0, 90.0, 0.0),
    ...
    part = flange.part.moved(bd.Location(TOOL_FLANGE_CENTER))
...
def _make_cable_guide():
    ...
    profile_plane = bd.Plane(origin=points[0], x_dir=(0.0, 1.0, 0.0), z_dir=tangent)

    with bd.BuildPart() as cable:
...
def _make_cable_clamp(label: str, center: tuple[float, float, float], angle_y: float):
    with bd.BuildPart() as clamp:
        bd.Box(12.0, 6.0, 5.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        bd.Cylinder(
            radius=2.2,
            height=14.0,
            rotation=(90.0, 0.0, 0.0),
    ...
    part = clamp.part.moved(bd.Location(center, (0.0, angle_y, 0.0)))
...
@step(out="../STEP/six_axis_industrial_robot_arm.step")
def six_axis_industrial_robot_arm():
