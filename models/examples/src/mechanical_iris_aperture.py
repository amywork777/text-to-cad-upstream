...
def _make_base_ring():
    with bd.BuildPart() as base:
        ...
        with bd.Locations(bd.Location((0.0, 0.0, -0.75))):
            bd.Cylinder(
                radius=BASE_ID / 2.0,
        ...
        for index in range(BASE_MOUNT_HOLE_COUNT):
            ...
            with bd.Locations(bd.Location((x_pos, y_pos, -0.75))):
                bd.Cylinder(
            ...
            with bd.Locations(bd.Location((x_pos, y_pos, BASE_TOP_Z - BASE_COUNTERSINK_DEPTH))):
                bd.Cone(
        ...
        with bd.BuildSketch(bd.Plane.XY.offset(BASE_TOP_Z - 0.22)):
            for index in range(72):
                ...
                bd.Polygon(_tick_rectangle_points(angle, inner_radius, outer_radius, width), align=None)
        bd.extrude(amount=0.36, mode=bd.Mode.SUBTRACT)
...
def _make_tick_marks() -> list[object]:
    ...
    for index in range(72):
        ...
        outer_radius = 79.35
        width = 0.72 if major else 0.44
        with bd.BuildPart() as tick:
            with bd.BuildSketch(bd.Plane.XY.offset(BASE_TOP_Z - 0.18)):
                bd.Polygon(_tick_rectangle_points(angle, inner_radius, outer_radius, width), align=None)
            bd.extrude(amount=0.12)
...
def _make_retaining_ring():
    ...
    with bd.BuildPart() as retaining:
        with bd.BuildSketch(bd.Plane.XY.offset(RETAINING_Z_BOTTOM)):
            bd.Polygon(
                _tooth_profile(
        ...
        bd.extrude(amount=RETAINING_HEIGHT)

        with bd.Locations(bd.Location((0.0, 0.0, RETAINING_Z_BOTTOM - 0.5))):
            bd.Cylinder(
...
def _make_iris_blade(index: int):
    ...
    with bd.BuildPart() as blade:
        with bd.BuildSketch(bd.Plane.XY.offset(z_bottom)):
            bd.Polygon(_blade_outline_points(blade_angle), align=None)
        bd.extrude(amount=BLADE_THICKNESS)

        with bd.Locations(bd.Location((pivot_x, pivot_y, z_bottom - 0.2))):
            bd.Cylinder(
...
def _make_actuating_ring():
    with bd.BuildPart() as actuator:
        with bd.Locations(bd.Location((0.0, 0.0, ACTUATING_Z_BOTTOM))):
            bd.Cylinder(
                radius=ACTUATING_OD / 2.0,
                height=ACTUATING_HEIGHT,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN),
            )
        with bd.Locations(bd.Location((0.0, 0.0, ACTUATING_Z_BOTTOM - 0.5))):
            bd.Cylinder(
        ...
        with bd.BuildSketch(bd.Plane.XY.offset(ACTUATING_Z_BOTTOM - 0.2)):
            for index in range(BLADE_COUNT):
                blade_angle = tau * index / BLADE_COUNT
                slot_angle = blade_angle + BLADE_DRIVE_PIN_LOCAL_ANGLE
                bd.Polygon(_curved_slot_points(slot_angle), align=None)
        bd.extrude(amount=ACTUATING_HEIGHT + 0.4, mode=bd.Mode.SUBTRACT)
...
def _make_pivot_screw(index: int):
    ...
    with bd.BuildPart() as screw:
        ...
        with bd.Locations(bd.Location((0.0, 0.0, shaft_height))):
            bd.Cylinder(
                radius=2.5,
                height=head_height,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN),
            )
        with bd.BuildSketch(bd.Plane.XY.offset(shaft_height + head_height - 0.72)):
            bd.RegularPolygon(radius=1.35, side_count=6, major_radius=True, rotation=30.0)
        bd.extrude(amount=0.82, mode=bd.Mode.SUBTRACT)
    ...
    part = part.moved(bd.Location((x_pos, y_pos, BLADE_Z_BOTTOM - 0.1)))
...
def _make_drive_pin(index: int):
    ...
    part = part.moved(bd.Location((x_pos, y_pos, BLADE_Z_BOTTOM - 0.05)))
...
def _make_front_socket_screw(index: int):
    ...
    with bd.BuildPart() as screw:
        ...
        with bd.BuildSketch(bd.Plane.XY.offset(shaft_height + FRONT_SCREW_HEAD_HEIGHT - socket_depth)):
            bd.RegularPolygon(radius=2.05, side_count=6, major_radius=True, rotation=30.0)
        bd.extrude(amount=socket_depth + 0.15, mode=bd.Mode.SUBTRACT)
    ...
    part = part.moved(bd.Location((x_pos, y_pos, RETAINING_Z_BOTTOM - 0.1)))
    return _set_metadata(part, f"front_socket_head_cap_screw_{index + 1:02d}_gunmetal", GUNMETAL)


def _make_aperture_shadow():
    with bd.BuildPart() as shadow:
        with bd.Locations(bd.Location((0.0, 0.0, BLADE_Z_BOTTOM - 0.22))):
            bd.Cylinder(
                radius=18.0,
                height=0.12,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN),
...
@step(out="../STEP/mechanical_iris_aperture.step")
def mechanical_iris_aperture():
