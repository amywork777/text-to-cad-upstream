...
def _make_blade(index: int):
    ...
    with bd.BuildPart() as blade:
        with bd.BuildSketch(bd.Plane.XY):
            bd.Polygon(_blade_outline(base_angle), align=None)
        bd.extrude(amount=BLADE_HEIGHT_ABOVE_BACKPLATE)

    part = blade.part.moved(bd.Location((0.0, 0.0, BACKPLATE_THICKNESS)))
    part.label = f"blade_{index + 1:02d}"
    return part


def _make_backplate():
    with bd.BuildPart() as backplate:
        bd.Cylinder(
    ...
    part = bd.fillet(outer_edges, radius=BACKPLATE_OUTER_FILLET_RADIUS)
...
@step(out="../STEP/centrifugal_impeller.step")
def centrifugal_impeller():
    ...
    with bd.BuildPart() as impeller:
        bd.add(backplate, mode=bd.Mode.ADD)

        with bd.Locations(bd.Location((0.0, 0.0, BACKPLATE_THICKNESS))):
            bd.Cylinder(
                radius=HUB_RADIUS,
        ...
        for blade_index in range(BLADE_COUNT):
            bd.add(_make_blade(blade_index), mode=bd.Mode.ADD)
    ...
    if root_edges:
        part = bd.fillet(root_edges, radius=BLADE_ROOT_FILLET_RADIUS)
