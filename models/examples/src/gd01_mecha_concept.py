...
upright stance (legs straight, arms hanging).

Every top-level child of the root compound is one rigid animation group,
addressable by child order (#o1.N), so keep FEATURE_ORDER in sync with the
list built in gd01_mecha_concept(). The retired `.params.js` sidecar that
drove the biped-to-quadruped transformation, walking, cockpit access, and the
exploded reveal was dropped with the pose framework; re-author choreography as
a `.anim.js` module (see the cad skill's kinematics reference).
"""
...
GLOSS_RED = bd.Color(*_arm_rgb, 1.0)
DEEP_RED = bd.Color(*(c * 0.62 for c in _arm_rgb), 1.0)
MATTE_BLACK = bd.Color(0.030, 0.032, 0.036, 1.0)
GRAPHITE = bd.Color(0.085, 0.09, 0.10, 1.0)
DARK_STEEL = bd.Color(0.16, 0.17, 0.185, 1.0)
SILVER = bd.Color(0.76, 0.78, 0.80, 1.0)
BRIGHT_SILVER = bd.Color(0.86, 0.87, 0.89, 1.0)
RUBBER = bd.Color(0.05, 0.05, 0.055, 1.0)
PILOT_GRAY = bd.Color(0.30, 0.31, 0.33, 1.0)
SEAT_BLACK = bd.Color(0.07, 0.07, 0.08, 1.0)
LENS_BLUE = bd.Color(0.25, 0.48, 0.85, 0.55)
LENS_AMBER = bd.Color(0.95, 0.55, 0.12, 0.55)
VISOR_TINT = bd.Color(0.62, 0.78, 0.92, 0.30)
...
def _style(part, label: str, color: bd.Color):
    part.label = label
    part.color = color
    return part
...
def _safe_fillet(edges, radius: float) -> None:
    ...
    if not selected or radius <= 0.0:
        return
    try:
        bd.fillet(selected, radius=radius)
    except Exception:
        pass
...
def _rounded_box(
    ...
    with bd.BuildPart() as builder:
        bd.Box(*size, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        if radius > 0.0:
            _safe_fillet(builder.edges(), radius)
    return _style(builder.part.moved(bd.Location(center, rotation)), label, color)
...
def _axis_cylinder(
    ...
    with bd.BuildPart() as builder:
        bd.Cylinder(radius=radius, height=length, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        if fillet_radius > 0.0:
            _safe_fillet(builder.edges(), fillet_radius)
    return _style(builder.part.moved(bd.Location(center, _AXIS_ROT[axis])), label, color)


def _tube(label: str, color: bd.Color, points: list[tuple[float, float, float]], radius: float):
    path = bd.Edge.make_spline(points)
    tangent = (
        points[1][0] - points[0][0],
        points[1][1] - points[0][1],
        points[1][2] - points[0][2],
    )
    profile_plane = bd.Plane(origin=points[0], x_dir=(0.0, 0.0, 1.0), z_dir=tangent)
    with bd.BuildPart() as builder:
...
def _mirror_part(part):
    ...
    color = part.color
    flipped = bd.mirror(part, about=bd.Plane.XZ)
    swapped = (
...
def _mirror_group(group_label: str, parts: list) -> bd.Compound:
...
def _pelvis_armor_parts() -> list:
    ...
    with bd.BuildPart() as front:
        bd.Box(58.0 * S, HIP_Y * 2.0 - 160.0 * S, 260.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        # horizontal service seam groove across the plate face
        with bd.Locations(bd.Location((26.0 * S, 0.0, 30.0 * S))):
            bd.Box(10.0 * S, HIP_Y * 2.0 - 150.0 * S, 5.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(front.edges(), 14.0 * S)
    parts.append(
        _style(
            front.part.moved(bd.Location((200.0 * S, 0.0, HIP_Z + 80.0 * S))),
...
def _torso_armor_parts() -> list:
    ...
    for side_name, side in (("left", 1.0), ("right", -1.0)):
        with bd.BuildPart() as panel:
            bd.Box(430.0 * S, 70.0 * S, 540.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
            # cooling vent louvers cut into the upper half of the chest cheek
            for index in range(4):
                with bd.Locations(bd.Location((-120.0 * S + index * 62.0 * S, 0.0, 170.0 * S))):
                    bd.Box(30.0 * S, 90.0 * S, 90.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
            # vertical service seam
            with bd.Locations(bd.Location((90.0 * S, side * 32.0 * S, -120.0 * S))):
                bd.Box(9.0 * S, 8.0 * S, 300.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
            _safe_fillet(panel.edges(), 16.0 * S)
        parts.append(
            _style(
                panel.part.moved(bd.Location((60.0 * S, side * (HALF_W + 36.0 * S), HIP_Z + 560.0 * S))),
    ...
    with bd.BuildPart() as back:
        bd.Box(140.0 * S, W - 180.0 * S, 720.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        for index in range(6):
            with bd.Locations(bd.Location((-40.0 * S, 0.0, -240.0 * S + index * 96.0 * S))):
                bd.Box(90.0 * S, W - 320.0 * S, 30.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(back.edges(), 20.0 * S)
    parts.append(
        _style(
            back.part.moved(bd.Location((-430.0 * S, 0.0, HIP_Z + 520.0 * S))),
...
def _pilot_parts() -> list:
    ...
    parts.append(
        _style(
            head.part.moved(bd.Location((SEAT_X - 10.0 * S, 0.0, pan_top + 640.0 * S))),
...
def _actuator_pod_parts(
    ...
    housing = _style(
        pod.part.moved(bd.Location(center, _AXIS_ROT["y"])),
...
def _left_thigh_armor_parts() -> list:
    ...
    with bd.BuildPart() as shell:
        bd.Box(120.0 * S, 250.0 * S * a, THIGH_LEN * 0.76, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        for seam_z in (-THIGH_LEN * 0.16, THIGH_LEN * 0.14):
            with bd.Locations(bd.Location((52.0 * S, 0.0, seam_z))):
                bd.Box(10.0 * S, 260.0 * S * a, 5.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(shell.edges(), 26.0 * S)
    parts.append(
        _style(
            shell.part.moved(bd.Location((190.0 * S, HIP_Y, mid_z + 20.0 * S))),
...
def _left_shin_core_parts() -> list:
    ...
    parts = [
        _style(
            column.part.moved(bd.Location((0.0, HIP_Y, mid_z))),
...
def _left_shin_armor_parts() -> list:
    ...
    parts = []
    with bd.BuildPart() as guard:
        bd.Box(110.0 * S, 230.0 * S * a, SHIN_LEN * 0.78, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        for index in range(4):
            with bd.Locations(bd.Location((48.0 * S, 0.0, -SHIN_LEN * 0.24 + index * 52.0 * S))):
                bd.Box(14.0 * S, 150.0 * S * a, 20.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        with bd.Locations(bd.Location((44.0 * S, 0.0, SHIN_LEN * 0.24))):
            bd.Box(10.0 * S, 240.0 * S * a, 5.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(guard.edges(), 24.0 * S)
    parts.append(
        _style(
            guard.part.moved(bd.Location((125.0 * S, HIP_Y, mid_z + 10.0 * S))),
...
def _left_foot_pad_parts() -> list:
    ...
    with bd.BuildPart() as pad:
        bd.Box(PAD_LEN, PAD_WID, PAD_TH, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN))
        _safe_fillet(pad.edges(), 22.0 * S)
    parts.append(
        _style(
            pad.part.moved(bd.Location((50.0 * S, HIP_Y, 0.0))),
...
def _left_upper_arm_armor_parts() -> list:
    ...
    with bd.BuildPart() as panel:
        bd.Box(90.0 * S, 190.0 * S * a, UPPER_LEN * 0.62, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        with bd.Locations(bd.Location((40.0 * S, 0.0, UPPER_LEN * 0.1))):
            bd.Box(9.0 * S, 200.0 * S * a, 5.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(panel.edges(), 20.0 * S)
    parts.append(
        _style(
            panel.part.moved(bd.Location((SHOULDER_X + 120.0 * S, ARM_Y, mid_z - 20.0 * S))),
...
def _left_forearm_core_parts() -> list:
    ...
    parts = [
        _style(
            column.part.moved(bd.Location((SHOULDER_X, ARM_Y, mid_z))),
...
def _left_forearm_armor_parts() -> list:
    ...
    parts = []
    with bd.BuildPart() as shell:
        bd.Box(96.0 * S, 180.0 * S, FORE_LEN * 0.6, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        with bd.Locations(bd.Location((42.0 * S, 0.0, -FORE_LEN * 0.08))):
            bd.Box(9.0 * S, 190.0 * S, 5.0 * S, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER), mode=bd.Mode.SUBTRACT)
        _safe_fillet(shell.edges(), 22.0 * S)
    parts.append(
        _style(
            shell.part.moved(bd.Location((SHOULDER_X + 105.0 * S, ARM_Y, mid_z + 10.0 * S))),
...
def _left_hand_pad_parts() -> list:
    ...
    parts.append(
        _style(
            dome.part.moved(bd.Location((SHOULDER_X, ARM_Y, pad_center_z - 40.0 * S))),
...
# Assembly
# ---------------------------------------------------------------------------
@step(out="../STEP/gd01_mecha_concept.step")
def gd01_mecha_concept():
