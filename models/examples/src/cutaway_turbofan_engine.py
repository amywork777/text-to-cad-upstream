...
def _make_radial_blade(
    *,
    ...
    part = bd.loft(faces, ruled=False)
    return _label(part, label, color)


def _make_fan_rotor() -> bd.Compound:
    parts = [
        _make_cylinder_x(
            ...
            x_end=37.0,
            radius=13.0,
            color=bd.Color(0.31, 0.38, 0.43, 1.0),
        )
    ]
    for index in range(24):
        parts.append(
            _make_radial_blade(
                ...
                axial_sweep=3.0,
                color=bd.Color(0.77, 0.82, 0.86, 1.0),
            )
        )
    return _make_compound("fan_rotor", parts)


def _make_compressor_rotor() -> bd.Compound:
    parts = []
    ...
    for stage_index, x_center in enumerate(stage_centers):
        parts.append(
            _make_cylinder_x(
                ...
                x_end=x_center + 2.0,
                radius=21.0,
                color=bd.Color(0.45, 0.50, 0.54, 1.0),
            )
        )
        ...
        for blade_index in range(18):
            parts.append(
                _make_radial_blade(
                    ...
                    axial_sweep=1.2,
                    color=bd.Color(0.69, 0.73, 0.76, 1.0),
                )
            )
    return _make_compound("compressor_rotor_four_stages", parts)


def _make_stator_vanes() -> bd.Compound:
    parts = []
        ...
        for vane_index in range(16):
            parts.append(
                _make_radial_blade(
                    ...
                    twist_tip_deg=-18.0,
                    axial_sweep=-0.8,
                    color=bd.Color(0.79, 0.65, 0.37, 1.0),
                )
            )
    return _make_compound("stator_vanes_three_rings", parts)


def _make_turbine_rotor() -> bd.Compound:
    parts = []
    for stage_index, x_center in enumerate([128.0, 145.0]):
        parts.append(
            _make_cylinder_x(
                ...
                x_end=x_center + 2.5,
                radius=23.0,
                color=bd.Color(0.49, 0.45, 0.41, 1.0),
            )
        )
        ...
        for blade_index in range(22):
            parts.append(
                _make_radial_blade(
                    ...
                    axial_sweep=2.2,
                    color=bd.Color(0.74, 0.59, 0.42, 1.0),
                )
            )
...
def _make_exhaust_cone() -> object:
    part = bd.loft(
        [
            bd.Face.make_surface(
                bd.Wire.make_circle(
                    18.0,
                    plane=bd.Plane((145.0, 0.0, 0.0), x_dir=(0.0, 1.0, 0.0), z_dir=(1.0, 0.0, 0.0)),
                )
            ),
            bd.Face.make_surface(
                bd.Wire.make_circle(
                    7.0,
                    plane=bd.Plane((178.0, 0.0, 0.0), x_dir=(0.0, 1.0, 0.0), z_dir=(1.0, 0.0, 0.0)),
                )
            ),
        ]
    )
    return _label(part, "exhaust_cone_taper_d36_to_d14", bd.Color(0.38, 0.40, 0.42, 1.0))
...
def _make_support_strut(index: int, angle_deg: float, *, trimmed: bool = False) -> object:
    ...
    return _label(part, f"support_strut_{index}_{int(angle_deg)}deg{suffix}", bd.Color(0.33, 0.36, 0.38, 1.0))


def _make_support_struts() -> bd.Compound:
    parts = []
...
def _make_mounting_pylon() -> object:
    def pylon_face(z_pos: float, x_width: float, y_width: float, x_center: float) -> bd.Face:
        points = [
    ...
    return _label(part, "mounting_pylon_swept_aero_support", bd.Color(0.46, 0.49, 0.50, 1.0))
...
def _make_display_base() -> object:
    with bd.BuildPart() as base:
        ...
        for x_offset in (-50.0, 50.0):
            for y_pos in (-22.0, 22.0):
                ...
                with bd.Locations(bd.Location((x_pos - BASE_CENTER_X, y_pos, -0.5))):
                    bd.Cylinder(
                        radius=3.0,
                ...
                with bd.Locations(bd.Location((x_pos - BASE_CENTER_X, y_pos, BASE_THICKNESS))):
                    bd.Cone(
                        bottom_radius=3.0,
                        top_radius=5.5,
                        height=2.6,
    ...
    part = base.part.moved(bd.Location((BASE_CENTER_X, 0.0, BASE_BOTTOM_Z)))
    outer_edges = []
    for edge in part.edges():
        bbox = edge.bounding_box()
        center = bbox.center()
    ...
    return _label(part, "display_base_rounded_rectangle_countersunk_holes", bd.Color(0.22, 0.24, 0.25, 1.0))


@step(out="../STEP/cutaway_turbofan_engine.step")
def cutaway_turbofan_engine():
    ...
    bodies = [
        ...
        _make_cylinder_x(
            ...
            x_end=165.0,
            radius=4.0,
            color=bd.Color(0.18, 0.20, 0.22, 1.0),
        ),
        _make_stator_vanes(),
        _make_exhaust_cone(),
