...
def _annular_cylinder(
    *,
    ...
    with bd.BuildPart() as ring:
        with bd.Locations(bd.Location((0.0, 0.0, z_bottom))):
            bd.Cylinder(
                radius=outer_radius,
                height=height,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN),
            )
        with bd.Locations(bd.Location((0.0, 0.0, z_bottom - 0.75))):
            bd.Cylinder(
                radius=inner_radius,
                height=height + 1.5,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.MIN),
...
def _make_paper_cup_body() -> object:
    ...
    with bd.BuildPart() as cup:
        ...
        with bd.Locations(bd.Location((0.0, 0.0, BODY_BOTTOM_THICKNESS))):
            bd.Cone(
...
def _make_bottom_foot_ring() -> object:
    with bd.BuildPart() as foot:
        with bd.Locations(bd.Location((0.0, 0.0, 3.2))):
            bd.Torus(major_radius=BODY_BOTTOM_RADIUS - 2.0, minor_radius=1.25)
...
def _make_lid_skirt_parts() -> list[object]:
    ...
    for label, z_pos, major_radius, minor_radius in [
        ...
        with bd.BuildPart() as bead:
            with bd.Locations(bd.Location((0.0, 0.0, z_pos))):
                bd.Torus(major_radius=major_radius, minor_radius=minor_radius)
...
def _make_lid_top() -> list[object]:
    ...
    with bd.BuildPart() as dome:
        with bd.Locations(bd.Location((0.0, 0.0, 115.5))):
            bd.Cone(
                bottom_radius=42.8,
                top_radius=39.0,
                height=9.4,
    ...
    with bd.BuildPart() as top_outer_ring:
        with bd.Locations(bd.Location((0.0, 0.0, 126.9))):
            bd.Torus(major_radius=36.4, minor_radius=0.85)
    parts.append(_set_metadata(top_outer_ring.part, "fine_raised_outer_top_emboss_ring", LID_HIGHLIGHT))

    with bd.BuildPart() as top_inner_ring:
        with bd.Locations(bd.Location((0.0, 0.0, 127.15))):
            bd.Torus(major_radius=18.0, minor_radius=0.38)
...
def _make_sip_feature() -> list[object]:
    parts: list[object] = []
    top_plane = bd.Plane.XY.offset(LID_TOP_Z)

    with bd.BuildPart() as raised_sip:
        with bd.BuildSketch(top_plane):
            with bd.Locations((11.0, 18.4)):
                bd.SlotOverall(width=31.0, height=12.2, rotation=-9.0)
        bd.extrude(amount=1.2)
    parts.append(_set_metadata(raised_sip.part, "raised_oval_sipping_platform_on_lid", LID_HIGHLIGHT))

    with bd.BuildPart() as sip_opening:
        with bd.BuildSketch(bd.Plane.XY.offset(LID_TOP_Z + 1.22)):
            with bd.Locations((15.2, 20.0)):
                bd.SlotOverall(width=16.0, height=4.0, rotation=-9.0)
        bd.extrude(amount=0.45)
    parts.append(_set_metadata(sip_opening.part, "dark_recessed_sip_slot_opening", DARK_OPENING))

    with bd.BuildPart() as nose_ridge:
        with bd.BuildSketch(bd.Plane.XY.offset(LID_TOP_Z + 0.7)):
            with bd.Locations((0.5, 7.5)):
                bd.RectangleRounded(width=10.0, height=22.0, radius=2.0, rotation=-8.0)
        bd.extrude(amount=0.75)
...
def _make_top_embossing() -> list[object]:
    parts: list[object] = []
    emboss_plane = bd.Plane.XY.offset(LID_TOP_Z + 0.18)

    with bd.BuildPart() as caution_text:
        with bd.BuildSketch(emboss_plane):
            with bd.Locations((-2.5, -10.5)):
                bd.Text(
                    "CAUTION HOT",
                    font_size=4.0,
                    text_align=(bd.TextAlign.CENTER, bd.TextAlign.CENTER),
                    rotation=3.0,
                )
            with bd.Locations((-4.0, 6.0)):
                bd.Text(
                    "PLEASE RECYCLE",
                    font_size=3.0,
                    text_align=(bd.TextAlign.CENTER, bd.TextAlign.CENTER),
    ...
    for index, (x_pos, y_pos, width, height, rotation) in enumerate(
        ...
        with bd.BuildPart() as tick:
            with bd.BuildSketch(bd.Plane.XY.offset(LID_TOP_Z + 0.16)):
                with bd.Locations((x_pos, y_pos)):
                    bd.Rectangle(width=width, height=height, rotation=rotation)
            bd.extrude(amount=0.24)
...
def _make_curved_decal_cell(
    *,
    ...
    compound = bd.Compound(obj=segment_parts, children=segment_parts, label=label)
...
def _make_surface_decal_box(
    *,
    ...
    box = bd.Solid.make_box(width, thickness, height)
    box = box.moved(bd.Location((-width / 2.0, -thickness / 2.0, -height / 2.0)))
    box = box.moved(bd.Location(center, (0.0, 0.0, degrees(angle))))
    return _set_metadata(box, label, color)


@step(out="../STEP/photo_coffee_cup.step")
def photo_coffee_cup():
