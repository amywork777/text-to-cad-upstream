...
from lib.part_common import rounded_triangle_points, safe_fillet
...
def _make_lug(y_min: float, label: str):
    with bd.BuildPart() as lug:
        with bd.BuildSketch(bd.Plane.XZ):
            bd.Polygon(_lug_profile_points(), align=None)
        bd.extrude(amount=LUG_THICKNESS_Y)

    part = lug.part.moved(bd.Location((0.0, y_min, 0.0)))
    part.label = label
    return part
...
def _make_rib(sign: float):
    ...
    with bd.BuildPart() as rib:
        ...
        bd.extrude(amount=RIB_THICKNESS_X)

    part = rib.part.moved(bd.Location((-RIB_THICKNESS_X / 2.0, 0.0, 0.0)))
...
def _subtract_rounded_triangle(vertices: list[tuple[float, float]]):
    with bd.BuildSketch(bd.Plane.XY) as cutout:
        bd.Polygon(
            rounded_triangle_points(vertices, radius=CUTOUT_CORNER_RADIUS, segments=5),
            align=None,
        )
    bd.extrude(cutout.sketch, amount=BASE_THICKNESS + 2.0, mode=bd.Mode.SUBTRACT)


@step(out="../STEP/clevis_bracket_lightening_cutouts.step")
def clevis_bracket_lightening_cutouts():
    ...
    with bd.BuildPart() as bracket:
        ...
        bd.add(_make_lug(positive_lug_y_min, "positive_y_clevis_lug"), mode=bd.Mode.ADD)
        bd.add(_make_lug(negative_lug_y_min, "negative_y_clevis_lug"), mode=bd.Mode.ADD)
        bd.add(_make_rib(1.0), mode=bd.Mode.ADD)
        bd.add(_make_rib(-1.0), mode=bd.Mode.ADD)

        with bd.Locations(bd.Location((0.0, 0.0, CLEVIS_HOLE_CENTER_Z))):
            bd.Cylinder(
