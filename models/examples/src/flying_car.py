...
def lofted_superellipse(label: str, stations: Sequence[tuple[float, float, float, float]], color_name: str) -> bd.Shape:
    faces = [superellipse_section(*station) for station in stations]
    return style(softened(bd.loft(faces, ruled=False), 0.75), label, color_name)


def cylinder_x(label: str, center: Vector3, radius: float, length: float, color_name: str) -> bd.Shape:
    return style(bd.Cylinder(radius, length, rotation=(0.0, 90.0, 0.0)).moved(bd.Location(center)), label, color_name)


def cylinder_y(label: str, center: Vector3, radius: float, length: float, color_name: str) -> bd.Shape:
    return style(bd.Cylinder(radius, length, rotation=(90.0, 0.0, 0.0)).moved(bd.Location(center)), label, color_name)
...
def annular_z(label: str, center: Vector3, outer_radius: float, inner_radius: float, height: float, color_name: str) -> bd.Shape:
    outer = bd.Cylinder(outer_radius, height).moved(bd.Location(center))
    inner = bd.Cylinder(inner_radius, height + 3.0).moved(bd.Location(center))
    return style(softened(outer - inner, 0.65), label, color_name)


def annular_x(label: str, center: Vector3, outer_radius: float, inner_radius: float, length: float, color_name: str) -> bd.Shape:
    outer = bd.Cylinder(outer_radius, length, rotation=(0.0, 90.0, 0.0)).moved(bd.Location(center))
    inner = bd.Cylinder(inner_radius, length + 3.0, rotation=(0.0, 90.0, 0.0)).moved(bd.Location(center))
    return style(softened(outer - inner, 0.65), label, color_name)


def annular_y(label: str, center: Vector3, outer_radius: float, inner_radius: float, length: float, color_name: str) -> bd.Shape:
    outer = bd.Cylinder(outer_radius, length, rotation=(90.0, 0.0, 0.0)).moved(bd.Location(center))
    inner = bd.Cylinder(inner_radius, length + 3.0, rotation=(90.0, 0.0, 0.0)).moved(bd.Location(center))
    return style(softened(outer - inner, 0.55), label, color_name)


def prism_from_xz(label: str, y_center: float, points_xz: Sequence[tuple[float, float]], thickness: float, color_name: str) -> bd.Shape:
    faces = []
    for y_pos in (y_center - thickness / 2.0, y_center + thickness / 2.0):
        points = [(x_pos, y_pos, z_pos) for x_pos, z_pos in points_xz]
        faces.append(bd.Face.make_surface(bd.Wire.make_polygon(points, close=True)))
    return style(softened(bd.loft(faces, ruled=True), 0.45), label, color_name)


def prism_from_yz(label: str, x_center: float, points_yz: Sequence[tuple[float, float]], thickness: float, color_name: str) -> bd.Shape:
    faces = []
    for x_pos in (x_center - thickness / 2.0, x_center + thickness / 2.0):
        points = [(x_pos, y_pos, z_pos) for y_pos, z_pos in points_yz]
        faces.append(bd.Face.make_surface(bd.Wire.make_polygon(points, close=True)))
    return style(softened(bd.loft(faces, ruled=True), 0.35), label, color_name)


def body_shell() -> bd.Compound:
    fuselage = lofted_superellipse("one_piece_aero_road_car_fuselage", BODY_STATIONS, "pearl_body")
    belly = style(
        softened(bd.Box(225.0, 42.0, 8.0).moved(bd.Location((-18.0, 0.0, 18.0))), 1.2),
    ...
    splitter = style(
        softened(bd.Box(42.0, 54.0, 3.6).moved(bd.Location((172.0, 0.0, 32.0))), 0.8),
        "front_aero_splitter_under_nose",
        "graphite",
    )
    rear_diffuser = style(
        softened(bd.Box(58.0, 68.0, 5.0).moved(bd.Location((-167.0, 0.0, 27.0))), 0.8),
        "rear_diffuser_plane",
        "graphite",
    )
...
def wing_panel(label: str, side: float) -> bd.Shape:
    base_points = [
        (74.0, 43.0),
        (96.0, 138.0),
        (-92.0, 128.0),
    ...
    with bd.BuildPart() as wing:
        with bd.BuildSketch(bd.Plane.XY):
            bd.Polygon(points, align=None)
        bd.extrude(amount=5.2)
    panel = wing.part.moved(bd.Location((0.0, 0.0, 51.5)))
    return style(softened(panel, 0.7), label, "brushed_aluminum")


def stabilizers() -> bd.Compound:
...
def lifting_wings() -> bd.Compound:
    center_fairing = style(
        softened(bd.Box(120.0, 96.0, 7.0).moved(bd.Location((-2.0, 0.0, 53.0))), 1.0),
...
def rotor_blade_z(label: str, center: Vector3, angle_deg: float, color_name: str) -> bd.Shape:
    root_radius = 9.0
    ...
    with bd.BuildPart() as blade:
        with bd.BuildSketch(bd.Plane.XY):
            bd.Polygon(corners, align=None)
        bd.extrude(amount=1.8)
    return style(blade.part.moved(bd.Location((0.0, 0.0, center[2] - 0.9))), label, color_name)


def lift_duct_module(label: str, center: Vector3, front: bool, side: float) -> bd.Compound:
    ...
    parts: list[bd.Shape] = [
        ...
        style(
            bd.Torus(DUCT_OUTER_RADIUS - 2.4, 1.3).moved(bd.Location((center[0], center[1], center[2] + DUCT_HEIGHT / 2.0))),
...
def rear_blade_x(label: str, center: Vector3, angle_deg: float) -> bd.Shape:
    root_radius = 7.0
    tip_radius = 20.5
    root_width = 6.0
    tip_width = 2.8
...
def wheel_module(label: str, x_pos: float, side: float) -> bd.Compound:
    y_pos = side * 61.0
    wheel_center = (x_pos, y_pos, 20.0)
    side_label = "port" if side > 0 else "starboard"
    axle_end = (x_pos, side * 45.0, 20.0)
    fairing = style(
        softened(bd.Box(37.0, 17.0, 14.0).moved(bd.Location((x_pos, side * 52.0, 31.0))), 2.0),
