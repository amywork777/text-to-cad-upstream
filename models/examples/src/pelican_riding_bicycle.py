...
def _style(shape: bd.Shape, label: str, color_name: str) -> bd.Shape:
    shape.label = label
    shape.color = bd.Color(*COLORS[color_name])
    return shape


def _group(label: str, children: Sequence[bd.Shape], color_name: str | None = None) -> bd.Compound:
...
def _softened(shape: bd.Shape, radius: float) -> bd.Shape:
    try:
        return bd.fillet(shape.edges(), radius=radius)
    except Exception:
        try:
            return bd.chamfer(shape.edges(), length=radius * 0.45)
        except Exception:
            return shape
...
def _oriented_box(
    ...
    *,
    ...
    shape = _transformed(bd.Box(size_x, size_y, size_z), center, axis_x, axis_y, axis_z)
    if radius > 0.0:
...
def _cylinder_between(label: str, p1: Vector3, p2: Vector3, radius: float, color_name: str) -> bd.Shape:
    ...
    length = _v_len(axis)
    if length <= 1e-6:
        return _style(bd.Sphere(radius).moved(bd.Location(p1)), label, color_name)
    ...
    return _style(bd.Shape(obj=wrapped), label, color_name)


def _cylinder_y(label: str, center: Vector3, radius: float, length: float, color_name: str) -> bd.Shape:
    return _style(bd.Cylinder(radius, length, rotation=(90, 0, 0)).moved(bd.Location(center)), label, color_name)


def _cylinder_x(label: str, center: Vector3, radius: float, length: float, color_name: str) -> bd.Shape:
    return _style(bd.Cylinder(radius, length, rotation=(0, 90, 0)).moved(bd.Location(center)), label, color_name)


def _ellipsoid(label: str, center: Vector3, radii: Vector3, color_name: str) -> bd.Shape:
    shape = bd.scale(bd.Sphere(1.0), by=radii).moved(bd.Location(center))
    return _style(shape, label, color_name)
...
def _flat_cone_x(
    ...
    cone = bd.Cone(base_radius, tip_radius, length, rotation=(0, 90, 0))
    shape = bd.scale(cone, by=(1.0, y_scale, z_scale)).moved(bd.Location(center))
    return _style(shape, label, color_name)


def _make_wheel(label: str, center: Vector3) -> bd.Compound:
    cx, cy, cz = center
    ...
    parts: list[bd.Shape] = [
        _style(
            bd.Torus(tire_major_radius, tire_minor_radius, rotation=(90, 0, 0)).moved(bd.Location(center)),
            f"{label}_continuous_black_tire",
            "rubber_black",
        ),
        _style(
            bd.Torus(rim_radius, 1.45, rotation=(90, 0, 0)).moved(bd.Location(center)),
            f"{label}_silver_rim",
            "rim_silver",
        ),
...
def _make_bicycle() -> bd.Compound:
    ...
    parts: list[bd.Shape] = [
        ...
        _style(_softened(bd.Box(24.0, 17.0, 5.0).moved(bd.Location((-20.0, 0.0, 97.0))), 1.2), "brown_saddle", "seat_brown"),
        _style(bd.Torus(7.5, 0.9, rotation=(90, 0, 0)).moved(bd.Location(bottom)), "crank_ring", "spoke_steel"),
    ...
    parts.extend(
        [
            ...
            _style(_softened(bd.Box(15.0, 4.0, 2.5).moved(bd.Location(pedal_front)), 0.7), "front_pedal_flat", "rubber_black"),
            _style(_softened(bd.Box(15.0, 4.0, 2.5).moved(bd.Location(pedal_back)), 0.7), "rear_pedal_flat", "rubber_black"),
        ]
    )
...
def _make_ground_shadow() -> bd.Compound:
    # Thin reference pads give the model a stable visual base without merging
    # into the wheels.
    front_pad = _style(_softened(bd.Box(92.0, 28.0, 1.0).moved(bd.Location((55.0, 0.0, -2.0))), 1.0), "front_wheel_ground_shadow", "rubber_black")
    rear_pad = _style(_softened(bd.Box(92.0, 28.0, 1.0).moved(bd.Location((-55.0, 0.0, -2.0))), 1.0), "rear_wheel_ground_shadow", "rubber_black")
    return _group("subtle_ground_shadow", [front_pad, rear_pad])


@step(out="../STEP/pelican_riding_bicycle.step")
def pelican_riding_bicycle() -> bd.Compound:
