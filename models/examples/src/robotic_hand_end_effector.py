...
def _add_teardrop_sketch(center: tuple[float, float], width: float, height: float) -> None:
    bd.Polygon(_teardrop_points(center, width, height), align=None)
    bd.Circle(width * 0.28, align=bd.Align.CENTER).locate(
        bd.Location((center[0], center[1] + height * 0.28, 0.0))
    )
...
def _make_palm_frame():
    ...
    with bd.BuildPart() as palm:
        ...
        bd.extrude(amount=PALM_THICKNESS)
        _safe_fillet(palm.edges(), 0.8)

        for center, width, height in openings:
            with bd.BuildSketch(bd.Plane.XY.offset(-0.8)):
                _add_teardrop_sketch(center, width, height)
            bd.extrude(amount=PALM_THICKNESS + 1.6, mode=bd.Mode.SUBTRACT)
...
def _make_palm_ribs():
    ...
    for name, center, width, height in openings:
        with bd.BuildPart() as rib:
            with bd.BuildSketch(bd.Plane.XY.offset(PALM_TOP_Z)):
                _add_teardrop_sketch(center, width + 6.0, height + 6.0)
            bd.extrude(amount=2.0)

            with bd.BuildSketch(bd.Plane.XY.offset(PALM_TOP_Z - 0.4)):
                _add_teardrop_sketch(center, width + 0.6, height + 0.6)
            bd.extrude(amount=3.0, mode=bd.Mode.SUBTRACT)
...
def _make_display_base():
    with bd.BuildPart() as base:
        with bd.BuildSketch(bd.Plane.XY):
            bd.RectangleRounded(110.0, 80.0, 6.0, align=(bd.Align.CENTER, bd.Align.CENTER))
        bd.extrude(amount=DISPLAY_BASE_THICKNESS)
        _safe_fillet(base.edges(), 3.0)

        for x_pos in (-44.0, 44.0):
            for y_pos in (-30.0, 30.0):
                with bd.Locations(bd.Location((x_pos, y_pos, -0.6))):
                    bd.Cylinder(
                        radius=2.35,
                ...
                with bd.Locations(bd.Location((x_pos, y_pos, DISPLAY_BASE_THICKNESS - 2.7))):
                    bd.Cone(
                        bottom_radius=2.35,
                        top_radius=5.1,
                        height=2.9,
    ...
    part = base.part.moved(bd.Location((0.0, -5.0, DISPLAY_BASE_BOTTOM_Z)))
...
def _make_support_post():
    ...
    part = post.part.moved(bd.Location((0.0, -45.0, DISPLAY_BASE_TOP_Z)))
...
def _make_wrist_gimbal():
    ...
    with bd.BuildPart() as wrist_cylinder:
        bd.Cylinder(
            radius=21.0,
            height=35.0,
    ...
    cylinder = wrist_cylinder.part.moved(bd.Location(WRIST_CENTER, (90.0, 0.0, 0.0)))
    ...
    roll_ring = ring.part.moved(bd.Location(WRIST_CENTER, (90.0, 0.0, 0.0)))
    ...
    for side, y_offset in (("front", 3.9), ("rear", -3.9)):
        for index in range(8):
            ...
            with bd.BuildPart() as bolt:
                bd.Cylinder(radius=2.15, height=1.5, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
                _safe_fillet(bolt.edges(), 0.5)
            part = bolt.part.moved(bd.Location((x_pos, WRIST_CENTER[1] + y_offset, z_pos), (90.0, 0.0, 0.0)))
            parts.append(
                _set_metadata(
                    part,
...
def _make_servo_housing(label: str, center: tuple[float, float, float], accent_color: bd.Color):
    with bd.BuildPart() as servo:
        bd.Box(12.0, 18.0, 8.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        _safe_fillet(servo.edges(), 1.2)
    body = servo.part.moved(bd.Location(center))
    body = _set_metadata(body, f"dark_graphite_servo_housing_{label}_12x18x8", DARK_GRAPHITE)

    with bd.BuildPart() as panel:
        bd.Box(8.0, 2.8, 0.6, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        _safe_fillet(panel.edges(), 0.25)
    accent = panel.part.moved(bd.Location((center[0], center[1] + 5.5, center[2] + 4.25)))
...
def _make_screw_head(label: str, center: tuple[float, float, float], radius: float, height: float, axis: str):
    with bd.BuildPart() as screw:
        bd.Cylinder(radius=radius, height=height, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
    ...
    part = screw.part.moved(bd.Location(center, rotations[axis]))
...
def _make_mechanical_link(label: str, length: float, start: tuple[float, float, float], flex: float, yaw: float):
    with bd.BuildPart() as link:
        ...
        bd.extrude(amount=FINGER_LINK_THICKNESS)

        with bd.Locations(bd.Location((0.0, length * 0.50, FINGER_LINK_THICKNESS - 0.55))):
            bd.Box(
        ...
        for x_pos in (-FINGER_LINK_WIDTH / 2.0 - 0.15, FINGER_LINK_WIDTH / 2.0 + 0.15):
            with bd.Locations(bd.Location((x_pos, length * 0.52, FINGER_LINK_THICKNESS * 0.52))):
                bd.Box(
                    1.15,
                    max(5.0, length * 0.52),
                    3.3,
    ...
    part = link.part.moved(bd.Location((0.0, 0.0, -FINGER_LINK_THICKNESS / 2.0)))
    part = part.moved(bd.Location(start, (-flex, 0.0, yaw)))
...
def _segment_end(start: tuple[float, float, float], length: float, flex: float, yaw: float) -> tuple[float, float, float]:
    edge = bd.Edge.make_line((0.0, 0.0, 0.0), (0.0, length, 0.0)).moved(
        bd.Location(start, (-flex, 0.0, yaw))
    )
...
def _make_hinge_pin(label: str, center: tuple[float, float, float], yaw: float):
    ...
    with bd.BuildPart() as pin:
        bd.Cylinder(radius=4.0, height=12.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        _safe_fillet(pin.edges(), 0.5)
    body = pin.part.moved(bd.Location(center, (0.0, 90.0, yaw)))
    ...
    for side, x_local in (("left", -6.8), ("right", 6.8)):
        ...
        with bd.BuildPart() as cap:
            bd.Cylinder(radius=5.0, height=1.5, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
            _safe_fillet(cap.edges(), 0.5)
        cap_part = cap.part.moved(bd.Location((x_pos, y_pos, center[2]), (0.0, 90.0, yaw)))
...
def _make_fingertip_pad(label: str, start: tuple[float, float, float], flex: float, yaw: float):
    ...
    with bd.BuildPart() as pad:
        with bd.BuildSketch(bd.Plane.XY):
            bd.RectangleRounded(9.5, pad_length, 3.3, align=(bd.Align.CENTER, bd.Align.MIN))
        bd.extrude(amount=7.2)
        _safe_fillet(pad.edges(), 1.0)
    part = pad.part.moved(bd.Location((0.0, 0.0, -3.6)))
    part = part.moved(bd.Location(start, (-flex, 0.0, yaw)))
    return _set_metadata(part, f"matte_black_rubber_{label}_rounded_fingertip_pad", MATTE_BLACK)


def _make_thumb_saddle():
    with bd.BuildPart() as saddle:
        bd.Box(13.0, 20.0, 9.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
        _safe_fillet(saddle.edges(), 1.0)
    part = saddle.part.moved(bd.Location((-35.5, -4.0, PALM_TOP_Z + 1.5), (0.0, 0.0, -28.0)))
...
def _tube_profile_plane(start: tuple[float, float, float], next_point: tuple[float, float, float]) -> bd.Plane:
    tangent = (
        next_point[0] - start[0],
        next_point[1] - start[1],
        next_point[2] - start[2],
    )
    return bd.Plane(origin=start, x_dir=(1.0, 0.0, 0.0), z_dir=tangent)
...
def _make_tendon_tube(label: str, points: list[tuple[float, float, float]]):
    ...
    with bd.BuildPart() as tube:
        with bd.BuildSketch(_tube_profile_plane(points[0], points[1])):
            bd.Circle(1.0)
...
def _make_knuckle_yokes():
    ...
    for name, center in MAIN_FINGER_BASES.items():
        with bd.BuildPart() as yoke:
            bd.Box(12.0, 7.0, 7.0, align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER))
            bd.Cylinder(
                radius=4.25,
                height=13.0,
                align=(bd.Align.CENTER, bd.Align.CENTER, bd.Align.CENTER),
        ...
        part = yoke.part.moved(bd.Location((center[0], center[1] - 2.5, center[2] - 0.6)))
...
def _make_decorative_hex_fasteners():
    ...
    for index, (x_pos, y_pos, z_pos) in enumerate(
        ...
        with bd.BuildPart() as fastener:
            with bd.BuildSketch(bd.Plane.XY):
                bd.RegularPolygon(3.2, 6, rotation=30.0)
            bd.extrude(amount=0.9)
            _safe_fillet(fastener.edges(), 0.25)
        part = fastener.part.moved(bd.Location((x_pos, y_pos, z_pos)))
        parts.append(_set_metadata(part, f"polished_steel_display_base_hex_fastener_{index:02d}", POLISHED_STEEL))
    return parts


@step(out="../STEP/robotic_hand_end_effector.step")
def robotic_hand_end_effector():
