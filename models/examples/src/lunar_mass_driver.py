...
class MassDriverParams:
    ...
    Geometry parameters change the built STEP. The presentation parameters
    (`coil_glow_intensity`, `payload_release_angle`,
    `acceleration_visual_intensity`, `lunar_dust_level`, visibility toggles,
    `accent_color_palette`, `cutaway_fraction`, `exploded_distance`) are baked
    here: the retired `.params.js` sidecar that made them live viewer controls
    went with the pose framework. Re-author motion as a `.anim.js` module (see
    the cad skill's kinematics reference); note that a different
    `track_segment_count`, `service_gantry_count`, `radiator_panel_count`,
    `rover_count`, or `solar_farm_area` changes occurrence ids.
    """
...
COL_TERRAIN = bd.Color(0.30, 0.30, 0.31, 1.0)
COL_TERRAIN_DARK = bd.Color(0.22, 0.22, 0.23, 1.0)
COL_PAD = bd.Color(0.42, 0.42, 0.44, 1.0)
COL_WHITE = bd.Color(0.86, 0.87, 0.89, 1.0)      # matte white industrial modules
COL_WHITE_DIM = bd.Color(0.72, 0.73, 0.76, 1.0)
COL_STEEL = bd.Color(0.64, 0.65, 0.68, 1.0)      # stainless Starship-derived hardware
COL_STEEL_DARK = bd.Color(0.48, 0.49, 0.52, 1.0)
COL_RAIL = bd.Color(0.55, 0.57, 0.62, 1.0)
COL_BLACK = bd.Color(0.055, 0.055, 0.065, 1.0)   # radiator panels
COL_GOLD = bd.Color(0.85, 0.60, 0.16, 1.0)       # thermal blankets
COL_SOLAR = bd.Color(0.07, 0.11, 0.30, 1.0)
COL_GLASS = bd.Color(0.05, 0.75, 0.90, 0.45)     # control-room glazing
COL_DUST = bd.Color(1.00, 0.44, 0.14, 1.0)       # orange dust plumes (alpha set per level)
COL_EARTH = bd.Color(0.16, 0.36, 0.85, 1.0)
COL_EARTH_HAZE = bd.Color(0.92, 0.95, 1.00, 0.85)
...
def _rgba(rgb, alpha: float = 1.0) -> bd.Color:
    return bd.Color(rgb[0], rgb[1], rgb[2], alpha)


def _tinted(part, label: str, color: bd.Color):
    part.label = label
    part.color = color
    return part


def _grp(label: str, parts: list) -> bd.Compound:
    return bd.Compound(obj=parts, children=parts, label=label)


def _inst(base, label: str, loc: bd.Location):
    copy = base.moved(loc)
    copy.label = label
    return copy


def _yaw_pitch_loc(pos, pitch_deg: float = 0.0, yaw_deg: float = 0.0) -> bd.Location:
    """Translate, then yaw about +Z, then pitch the +X axis up by pitch_deg."""
    loc = bd.Location(pos)
    if yaw_deg:
        loc = loc * bd.Location((0, 0, 0), (0, 0, yaw_deg))
    if pitch_deg:
        # Rotating about +Y by -pitch maps +X toward +Z (pitches the nose up).
        loc = loc * bd.Location((0, 0, 0), (0, -pitch_deg, 0))
    return loc


# --------------------------------------------------------------------------
# Track centerline profile
# --------------------------------------------------------------------------
...
def _make_track_segment(p: MassDriverParams, seg_len: float, cutaway: bool) -> bd.Compound:
    ...
    bed = _tinted(
        bd.Box(seg_len, BED_WIDTH, BED_THICKNESS).moved(bd.Location((0, 0, -BED_THICKNESS / 2))),
        "track_bed_truss_module", COL_WHITE,
    )

    rail = bd.Box(seg_len, 260, 220)
    rail_left = _tinted(rail.moved(bd.Location((0, +p.rail_spacing / 2, 110))), "guide_rail_left", COL_RAIL)
    rail_right = _tinted(rail.moved(bd.Location((0, -p.rail_spacing / 2, 110))), "guide_rail_right", COL_RAIL)

    n_coil = max(1, p.coil_count_per_segment)
    coil_pitch = seg_len / n_coil
    coil = bd.Torus(COIL_MAJOR_R, COIL_MINOR_R).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    yoke = bd.Box(700, 5_400, 1_900)
    coils = []
    yokes = []
    for k in range(n_coil):
        x = -seg_len / 2 + coil_pitch * (k + 0.5)
        coils.append(_inst(coil, f"accelerator_coil_{k + 1:02d}", bd.Location((x, 0, COIL_CENTER_Z))))
        yokes.append(_inst(yoke, f"superconducting_magnet_housing_{k + 1:02d}", bd.Location((x, 0, 100))))
    ...
    shield = bd.Box(seg_len, 160, BED_THICKNESS + shield_h)
    shield_z = (shield_h - BED_THICKNESS) / 2
    shield_left = _tinted(shield.moved(bd.Location((0, +2_780, shield_z))), "dust_shield_left", COL_WHITE)
    shield_right = _tinted(shield.moved(bd.Location((0, -2_780, shield_z))), "dust_shield_right", COL_WHITE)

    tray_parts = [
        _tinted(bd.Box(seg_len, 700, 160).moved(bd.Location((0, -3_400, -650))), "cable_tray_channel", COL_STEEL_DARK)
    ]
    n_pipe = max(1, p.power_cable_density)
    pipe = bd.Cylinder(150, seg_len).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    for k in range(n_pipe):
        y = -3_400 + (k - (n_pipe - 1) / 2.0) * 320
        tray_parts.append(_inst(pipe, f"power_conduit_{k + 1:02d}", bd.Location((0, y, -450))))
        tray_parts[-1].color = _rgba(pal["conduit"])
    cable_tray = _grp("cable_tray", tray_parts)

    cool = bd.Cylinder(140, seg_len).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    cooling_left = _inst(cool, "cooling_loop_left", bd.Location((0, +2_400, 950)))
    cooling_left.color = COL_STEEL
    cooling_right = _inst(cool, "cooling_loop_right", bd.Location((0, -2_400, 950)))
    cooling_right.color = COL_STEEL

    mark = bd.Box(600, BED_WIDTH, 60)
    markings = _grp(
        "safety_markings",
        [
            _inst(mark, "deck_marking_fore", bd.Location((seg_len / 2 - 350, 0, 30))),
            _inst(mark, "deck_marking_aft", bd.Location((-seg_len / 2 + 350, 0, 30))),
        ],
    )
...
def _make_breech_terminal(p: MassDriverParams, pal: dict) -> bd.Compound:
    """Open loading apron behind s=0: deck, rails, backstop, buffers."""
    cx = -APRON_LENGTH / 2
    parts = [
        _tinted(bd.Box(APRON_LENGTH, BED_WIDTH, BED_THICKNESS)
                .moved(bd.Location((cx, 0, DECK_HEIGHT - BED_THICKNESS / 2))), "breech_apron_deck", COL_WHITE),
        _tinted(bd.Box(APRON_LENGTH, 260, 220)
                .moved(bd.Location((cx, +p.rail_spacing / 2, DECK_HEIGHT + 110))), "breech_rail_left", COL_RAIL),
        _tinted(bd.Box(APRON_LENGTH, 260, 220)
                .moved(bd.Location((cx, -p.rail_spacing / 2, DECK_HEIGHT + 110))), "breech_rail_right", COL_RAIL),
        _tinted(bd.Box(1_000, 5_400, 4_500)
                .moved(bd.Location((-APRON_LENGTH - 500, 0, DECK_HEIGHT + 1_450))), "breech_backstop_wall", COL_WHITE_DIM),
        _tinted(bd.Cylinder(320, 1_600).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                .moved(bd.Location((-APRON_LENGTH + 800, +p.rail_spacing / 2, DECK_HEIGHT + 700))),
                "breech_buffer_left", COL_STEEL_DARK),
        _tinted(bd.Cylinder(320, 1_600).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                .moved(bd.Location((-APRON_LENGTH + 800, -p.rail_spacing / 2, DECK_HEIGHT + 700))),
                "breech_buffer_right", COL_STEEL_DARK),
    ]
    stripe = _tinted(bd.Box(APRON_LENGTH, 300, 60)
                     .moved(bd.Location((cx, 0, DECK_HEIGHT + 30))), "breech_centerline_marking", _rgba(pal["marking"]))
    parts.append(stripe)
    return _grp("breech_loading_terminal", parts)


def _make_muzzle_terminal(p: MassDriverParams, pal: dict) -> bd.Compound:
    ...
    ring = _tinted(bd.Torus(2_900, 300).moved(bd.Location((0, 0, 0), (0, 90, 0))), "muzzle_exit_ring", COL_GOLD)
    ring = _inst(ring, "muzzle_exit_ring", _yaw_pitch_loc((x_e, 0, z_e + COIL_CENTER_Z), pitch_deg))
    ring.color = COL_GOLD
    halo = _inst(bd.Torus(3_300, 90).moved(bd.Location((0, 0, 0), (0, 90, 0))), "muzzle_marking_ring",
    ...
    leg = bd.Box(600, 600, leg_h)
    legs = [
        _inst(leg, "muzzle_support_leg_left", bd.Location((x_e, +2_900, leg_h / 2))),
        _inst(leg, "muzzle_support_leg_right", bd.Location((x_e, -2_900, leg_h / 2))),
    ]
    for one in legs:
        one.color = COL_WHITE_DIM
    xf, zf, pf = track_frame(p, p.track_length - 4_000.0)
    fin = bd.Box(3_600, 220, 1_100)
    fins = [
...
def make_track_group(p: MassDriverParams) -> bd.Compound:
...
def make_foundation_group(p: MassDriverParams) -> bd.Compound:
    ...
    pile = bd.Cylinder(350, p.foundation_anchor_depth)
    parts = []
    for j, s in enumerate(boundaries):
        ...
        pylon = bd.Box(800, 800, h)
        for side, y in (("left", 1_600.0), ("right", -1_600.0)):
            parts.append(_inst(pylon, f"foundation_pylon_{j + 1:02d}_{side}", bd.Location((x, y, h / 2))))
            parts[-1].color = COL_WHITE_DIM
            parts.append(_inst(pile, f"regolith_anchor_pile_{j + 1:02d}_{side}",
                               bd.Location((x, y, -p.foundation_anchor_depth / 2))))
...
def make_sled_group(p: MassDriverParams) -> bd.Compound:
    ...
    carriage_parts = [
        _tinted(bd.Box(p.sled_length, 2_400, PLATE_THICKNESS)
                .moved(bd.Location((0, 0, plate_mid))), "carriage_deck_plate", COL_WHITE),
    ]
    skid = bd.Box(1_600, 460, SKID_HEIGHT)
    for i, x in enumerate((-p.sled_length / 2 + 1_000, p.sled_length / 2 - 1_000)):
        for side, y in (("left", p.rail_spacing / 2), ("right", -p.rail_spacing / 2)):
            skid_inst = _inst(skid, f"levitation_skid_{i + 1}_{side}", bd.Location((x, y, skid_z)))
            skid_inst.color = _rgba(pal["coil"])
            carriage_parts.append(skid_inst)
    nose = bd.Cone(1_050, 120, 1_900).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    carriage_parts.append(_inst(nose, "sled_nose_fairing", bd.Location((p.sled_length / 2 + 950, 0, plate_mid))))
    carriage_parts[-1].color = COL_WHITE
    tail = bd.Cone(1_050, 320, 1_100).moved(bd.Location((0, 0, 0), (0, -90, 0)))
    carriage_parts.append(_inst(tail, "sled_tail_fairing", bd.Location((-p.sled_length / 2 - 550, 0, plate_mid))))
    carriage_parts[-1].color = COL_WHITE
    carriage = _grp("acceleration_carriage", carriage_parts)

    saddle = bd.Box(700, 2_200, SADDLE_HEIGHT)
    pin = bd.Box(300, 300, 700)
    axis_z = payload_axis_height(p)
    cradle_parts = []
    for i, x in enumerate((-1_800.0, 1_800.0)):
        cradle_parts.append(_inst(saddle, f"payload_saddle_{i + 1}",
                                  bd.Location((x, 0, plate_z0 + PLATE_THICKNESS + SADDLE_HEIGHT / 2))))
    ...
    body = _tinted(bd.Cylinder(r, length).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                   .moved(bd.Location((0, 0, axis_z))), "payload_canister_body", COL_STEEL)
    dome = _tinted(bd.Sphere(r, arc_size1=0).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                   .moved(bd.Location((length / 2, 0, axis_z))), "payload_canister_nose_dome", COL_STEEL)
    collar = _tinted(bd.Cylinder(r * 0.62, 260).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                     .moved(bd.Location((-length / 2 - 130, 0, axis_z))), "payload_docking_collar", COL_STEEL_DARK)
    # Bands sit proud of the skin by ~25 mm while keeping coil-aperture clearance.
    band = bd.Torus(r - 30, 55).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    band_a = _inst(band, "thermal_blanket_band_fore", bd.Location((length * 0.22, 0, axis_z)))
    band_a.color = COL_GOLD
    band_b = _inst(band, "thermal_blanket_band_aft", bd.Location((-length * 0.22, 0, axis_z)))
    band_b.color = COL_GOLD
    stripe = _inst(bd.Torus(r - 40, 40).moved(bd.Location((0, 0, 0), (0, 90, 0))),
                   "payload_marking_ring", bd.Location((-length * 0.40, 0, axis_z)))
    ...
    return _inst(sled, "levitated_payload_sled", bd.Location(sled_home_position(p)))
...
def _make_gantry(p: MassDriverParams, pal: dict) -> bd.Compound:
    ...
    """
    parts = [
        _tinted(bd.Box(3_000, 3_000, 2_800).moved(bd.Location((0, 0, 1_400))), "gantry_pedestal", COL_WHITE),
        _tinted(bd.Cylinder(700, 5_800).moved(bd.Location((0, 0, 2_800 + 2_900))), "gantry_slew_column", COL_WHITE_DIM),
        _tinted(bd.Box(900, 9_600, 700).moved(bd.Location((0, -GANTRY_PIVOT_Y + 1_800, 8_100))), "gantry_service_boom", COL_WHITE),
        _tinted(bd.Box(700, 700, 1_500).moved(bd.Location((0, -GANTRY_PIVOT_Y, 7_300))), "gantry_tool_head", _rgba(pal["marking"])),
        _tinted(bd.Box(800, 1_100, 900).moved(bd.Location((0, -GANTRY_PIVOT_Y + 4_000, 7_300))), "gantry_service_pod_inner", COL_GOLD),
        _tinted(bd.Box(800, 1_100, 900).moved(bd.Location((0, -GANTRY_PIVOT_Y + 6_800, 7_300))), "gantry_service_pod_outer", COL_STEEL_DARK),
    ]
    return _grp("service_gantry", parts)


def make_gantry_group(p: MassDriverParams) -> bd.Compound:
    pal = _palette(p)
    base = _make_gantry(p, pal)
    slew = min(1.0, max(0.0, p.gantry_position)) * GANTRY_RETRACT_DEG
    gantries = []
    for i, x in enumerate(gantry_positions(p)):
        loc = bd.Location((x, GANTRY_PIVOT_Y, 0)) * bd.Location((0, 0, 0), (0, 0, slew))
        gantries.append(_inst(base, f"service_gantry_{i + 1:02d}", loc))
    return _grp("service_gantry_row", gantries)


def make_loading_station_group(p: MassDriverParams) -> bd.Compound:
    """Breech-side platform and payload transfer crane (children: platform, crane)."""
    pal = _palette(p)
    plat_parts = [
        _tinted(bd.Box(9_000, 3_400, DECK_HEIGHT).moved(bd.Location((CRANE_X, -6_600, DECK_HEIGHT / 2))),
                "loading_platform_deck", COL_WHITE),
        _tinted(bd.Box(2_600, 3_000, 1_200).moved(bd.Location((CRANE_X - 5_800, -6_600, 600))),
                "loading_stair_lower", COL_WHITE_DIM),
        _tinted(bd.Box(2_600, 3_000, 2_400 - 1_200).moved(bd.Location((CRANE_X - 4_200, -6_600, 1_200 + 600))),
                "loading_stair_upper", COL_WHITE_DIM),
        _tinted(bd.Box(1_800, 1_800, 1_900).moved(bd.Location((CRANE_X + 3_200, -6_600, DECK_HEIGHT + 950))),
                "loading_control_kiosk", COL_GOLD),
        _tinted(bd.Box(9_000, 200, 200).moved(bd.Location((CRANE_X, -8_200, DECK_HEIGHT + 1_000))),
    ...
    crane_parts = [
        _tinted(bd.Box(1_100, 1_100, 9_100).moved(bd.Location((CRANE_X - 2_600, beam_y1, 4_550))),
                "crane_leg_north_fore", COL_WHITE),
        _tinted(bd.Box(1_100, 1_100, 9_100).moved(bd.Location((CRANE_X + 2_600, beam_y1, 4_550))),
                "crane_leg_north_aft", COL_WHITE),
        _tinted(bd.Box(1_100, 1_100, 9_100).moved(bd.Location((CRANE_X - 2_600, beam_y0, 4_550))),
                "crane_leg_south_fore", COL_WHITE),
        _tinted(bd.Box(1_100, 1_100, 9_100).moved(bd.Location((CRANE_X + 2_600, beam_y0, 4_550))),
                "crane_leg_south_aft", COL_WHITE),
        _tinted(bd.Box(1_300, beam_y1 - beam_y0 + 1_100, 900)
                .moved(bd.Location((CRANE_X - 2_600, beam_mid, 9_550))), "crane_bridge_beam_fore", COL_WHITE),
        _tinted(bd.Box(1_300, beam_y1 - beam_y0 + 1_100, 900)
                .moved(bd.Location((CRANE_X + 2_600, beam_mid, 9_550))), "crane_bridge_beam_aft", COL_WHITE),
        _tinted(bd.Box(6_800, 1_600, 700).moved(bd.Location((CRANE_X, 0, 9_450))), "crane_trolley", COL_STEEL_DARK),
        _tinted(bd.Box(4_400, 1_900, 420).moved(bd.Location((CRANE_X, 0, 7_700))), "crane_spreader_bar", COL_GOLD),
    ]
    crane = _grp("payload_transfer_crane", crane_parts)
    return _grp("payload_loading_station", [platform, crane])


def make_lander_group(p: MassDriverParams) -> bd.Compound:
    """Starship-derived cargo lander on its pad (stub marker if hidden)."""
    px, py = PAD_CENTER
    if not p.starship_lander_visibility:
        stub = _tinted(bd.Cylinder(600, 120).moved(bd.Location((px, py, 60))), "cargo_lander_omitted_marker", COL_PAD)
        return _grp("starship_cargo_lander", [stub])

    hull_r, hull_h = 4_500.0, 16_000.0
    base_z = 250.0 + 2_300.0  # pad top + leg stance height
    parts = [
        _tinted(bd.Cylinder(hull_r, hull_h).moved(bd.Location((px, py, base_z + hull_h / 2))),
                "lander_hull_barrel", COL_STEEL),
        _tinted(bd.Sphere(hull_r, arc_size1=0).moved(bd.Location((px, py, base_z + hull_h))),
                "lander_nose_dome", COL_STEEL),
        _tinted(bd.Cone(4_900, 4_500, 2_300).moved(bd.Location((px, py, base_z - 1_150))),
                "lander_engine_skirt", COL_STEEL_DARK),
        _tinted(bd.Torus(hull_r + 10, 240).moved(bd.Location((px, py, base_z + hull_h * 0.62))),
                "lander_thermal_blanket_band", COL_GOLD),
        # Cargo door and payload hand-off cradle face the service road (+Y side).
        _tinted(bd.Box(3_200, 500, 6_000).moved(bd.Location((px, py + hull_r - 100, base_z + hull_h * 0.42))),
                "lander_cargo_bay_door", COL_STEEL_DARK),
        _tinted(bd.Box(2_600, 2_400, 700).moved(bd.Location((px, py + hull_r + 1_600, base_z + 900))),
                "lander_payload_cradle", COL_GOLD),
    ]
    for k in range(3):
        ang = radians(90 + 120 * k)
        bx, by = px + 2_400 * cos(ang), py + 2_400 * sin(ang)
        parts.append(_tinted(bd.Cone(700, 260, 1_500).moved(bd.Location((bx, by, base_z - 2_050))),
    ...
    for k in range(4):
        ...
        leg = bd.Box(500, 500, 5_200).moved(bd.Location((0, 0, 0), (0, -24, 0))) \
            .moved(bd.Location((0, 0, 0), (0, 0, degrees(ang))))
        parts.append(_inst(leg, f"lander_leg_{k + 1}", bd.Location((lx, ly, 2_450))))
        parts[-1].color = COL_STEEL_DARK
        parts.append(_tinted(bd.Cylinder(750, 240).moved(bd.Location((lx + 950 * cos(ang), ly + 950 * sin(ang), 370))),
...
def make_tower_group(p: MassDriverParams) -> bd.Compound:
    pal = _palette(p)
    tx, ty = TOWER_POS
    h = p.control_tower_height
    parts = [
        _tinted(bd.Box(5_200, 5_200, 1_400).moved(bd.Location((tx, ty, 700))), "tower_base_bunker", COL_WHITE),
        _tinted(bd.Cylinder(1_800, h - 1_400).moved(bd.Location((tx, ty, 1_400 + (h - 1_400) / 2))),
                "tower_shaft", COL_WHITE),
        _tinted(bd.Cylinder(3_200, 2_600).moved(bd.Location((tx, ty, h + 1_300))), "tower_control_cab", COL_WHITE),
        _tinted(bd.Cylinder(3_260, 900).moved(bd.Location((tx, ty, h + 1_300))), "tower_window_band", COL_GLASS),
        _tinted(bd.Cone(3_200, 900, 1_100).moved(bd.Location((tx, ty, h + 2_600 + 550))), "tower_roof", COL_WHITE_DIM),
        _tinted(bd.Cylinder(120, 3_200).moved(bd.Location((tx, ty, h + 3_150 + 1_600))), "tower_antenna_mast", COL_STEEL),
        _tinted(bd.Sphere(360).moved(bd.Location((tx, ty, h + 3_150 + 3_300))), "tower_beacon", _rgba(pal["marking"])),
        _tinted(bd.Cone(1_150, 150, 700).moved(bd.Location((0, 0, 0), (0, 55, 0)))
                .moved(bd.Location((tx + 2_500, ty + 2_500, h + 2_900))), "tower_relay_dish", COL_GOLD),
    ]
...
def make_solar_group(p: MassDriverParams) -> bd.Compound:
    ...
    mast = _tinted(bd.Cylinder(320, 3_200).moved(bd.Location((0, 0, 1_600))), "wing_mast", COL_WHITE_DIM)
    panel = bd.Box(SOLAR_WING_W, SOLAR_WING_H, 140)
    panel = panel.moved(bd.Location((0, 0, 0), (-p.solar_panel_deploy_angle, 0, 0)))
    panel = _tinted(panel.moved(bd.Location((0, 0, 3_400))), "wing_panel", COL_SOLAR)
    wing = _grp("solar_wing", [mast, panel])

    skid = _grp("solar_combiner_skid", [
        _tinted(bd.Box(4_200, 2_600, 1_700).moved(bd.Location((SOLAR_X0 - 9_000, SOLAR_ROW_YS[0], 850))),
                "combiner_cabinet", COL_WHITE),
        _tinted(bd.Cylinder(160, 9_000).moved(bd.Location((0, 0, 0), (90, 0, 0)))
                .moved(bd.Location((SOLAR_X0 - 9_000, SOLAR_ROW_YS[0] - 6_000, 400))),
                "combiner_feeder_conduit", _rgba(pal["conduit"])),
    ])
    wings = [skid]
    for w, (x, y) in enumerate(solar_wing_centers(p)):
        wings.append(_inst(wing, f"solar_wing_{w + 1:02d}", bd.Location((x, y, 0))))
...
def make_radiator_group(p: MassDriverParams) -> bd.Compound:
    ...
    rack = _grp("radiator_manifold_rack", [
        _tinted(bd.Cylinder(260, length).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                .moved(bd.Location(((xs[0] + xs[-1]) / 2, RADIATOR_ROW_Y, RADIATOR_HINGE_Z))),
                "radiator_manifold_pipe", COL_STEEL),
        _tinted(bd.Box(length, 900, 500).moved(bd.Location(((xs[0] + xs[-1]) / 2, RADIATOR_ROW_Y, 250))),
    ...
    flat = bd.Box(5_200, 4_600, 120).moved(bd.Location((0, -2_300, 0)))
    tilted = flat.moved(bd.Location((0, 0, 0), (-p.radiator_panel_angle, 0, 0)))
    panels = [rack]
    for i, x in enumerate(xs):
        panels.append(_inst(tilted, f"radiator_panel_{i + 1:02d}",
                            bd.Location((x, RADIATOR_ROW_Y, RADIATOR_HINGE_Z))))
        panels[-1].color = COL_BLACK
    return _grp("radiator_array", panels)


def make_power_group(p: MassDriverParams) -> bd.Compound:
    """Cryo/power skids, main cyan trunk run, supports, and track risers."""
    pal = _palette(p)
    parts = []
    for i, x in enumerate((38_000.0, 66_000.0, 94_000.0)):
        skid_parts = [
            _tinted(bd.Box(6_000, 3_500, 2_200).moved(bd.Location((x, -16_000, 1_100))),
                    "skid_cabinet", COL_WHITE),
            _tinted(bd.Cylinder(750, 4_800).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                    .moved(bd.Location((x, -15_400, 2_950))), "skid_cryo_tank_fore", COL_GOLD),
            _tinted(bd.Cylinder(750, 4_800).moved(bd.Location((0, 0, 0), (0, 90, 0)))
                    .moved(bd.Location((x, -16_900, 2_950))), "skid_cryo_tank_aft", COL_STEEL),
            _tinted(bd.Cylinder(170, 10_400).moved(bd.Location((0, 0, 0), (90, 0, 0)))
                    .moved(bd.Location((x, -16_000 + 5_200 + 200, 400))), "skid_feeder_conduit", _rgba(pal["conduit"])),
        ]
        parts.append(_grp(f"cryo_power_skid_{i + 1:02d}", skid_parts))

    substation = _grp("breech_substation", [
        _tinted(bd.Box(4_600, 3_000, 2_400).moved(bd.Location((-16_000, TRUNK_Y, 1_200))), "substation_hall", COL_WHITE),
        _tinted(bd.Cylinder(900, 1_900).moved(bd.Location((-17_600, TRUNK_Y + 900, 3_350))), "substation_capacitor_a", COL_GOLD),
        _tinted(bd.Cylinder(900, 1_900).moved(bd.Location((-14_400, TRUNK_Y + 900, 3_350))), "substation_capacitor_b", COL_GOLD),
    ])
    ...
    n_pipe = max(1, p.power_cable_density)
    trunk_pipe = bd.Cylinder(180, trunk_len).moved(bd.Location((0, 0, 0), (0, 90, 0)))
    trunk_parts = []
    for k in range(n_pipe):
        y = TRUNK_Y + (k - (n_pipe - 1) / 2.0) * 520
        trunk_parts.append(_inst(trunk_pipe, f"trunk_conduit_{k + 1:02d}", bd.Location((trunk_cx, y, 330))))
        trunk_parts[-1].color = _rgba(pal["conduit"])
    support = bd.Box(900, n_pipe * 520 + 500, 260)
    seg_len = p.track_length / p.track_segment_count
    for j in range(14):
        x = -15_000.0 + j * 15_000.0
        trunk_parts.append(_inst(support, f"trunk_support_{j + 1:02d}", bd.Location((x, TRUNK_Y, 130))))
    ...
    for j in range(p.track_segment_count + 1):
        ...
        riser_parts.append(_tinted(bd.Cylinder(150, h).moved(bd.Location((x, TRUNK_Y, 330 + h / 2))),
                                   f"track_power_riser_{j + 1:02d}", _rgba(pal["conduit"])))
        jumper = bd.Cylinder(120, 1_800).moved(bd.Location((0, 0, 0), (90, 0, 0)))
        riser_parts.append(_tinted(jumper.moved(bd.Location((x, TRUNK_Y + 900, 330 + h - 300))),
...
def _make_rover(pal: dict) -> bd.Compound:
    parts = [
        _tinted(bd.Box(4_500, 2_600, 700).moved(bd.Location((0, 0, 950))), "rover_chassis", COL_WHITE),
        _tinted(bd.Box(3_400, 2_200, 240).moved(bd.Location((-300, 0, 1_420))), "rover_cargo_deck", COL_STEEL_DARK),
        _tinted(bd.Box(500, 500, 1_300).moved(bd.Location((1_800, 0, 1_950))), "rover_sensor_mast", COL_WHITE_DIM),
        _tinted(bd.Sphere(300).moved(bd.Location((1_800, 0, 2_750))), "rover_sensor_pod", COL_GLASS),
        _tinted(bd.Box(900, 2_400, 160).moved(bd.Location((-2_050, 0, 1_180))), "rover_radiator_flap", COL_BLACK),
    ]
    wheel = bd.Cylinder(550, 420).moved(bd.Location((0, 0, 0), (90, 0, 0)))
    for i, x in enumerate((-1_600.0, 0.0, 1_600.0)):
        for side, y in (("left", 1_500.0), ("right", -1_500.0)):
            parts.append(_inst(wheel, f"rover_wheel_{i + 1}_{side}", bd.Location((x, y, 550))))
            parts[-1].color = COL_TERRAIN_DARK
    return _grp("construction_rover", parts)


def make_rover_group(p: MassDriverParams) -> bd.Compound:
    pal = _palette(p)
    base = _make_rover(pal)
    rovers = []
    for k, (x, y, yaw) in enumerate(rover_home_poses(p)):
        rovers.append(_inst(base, f"construction_rover_{k + 1:02d}",
                            bd.Location((x, y, 0)) * bd.Location((0, 0, 0), (0, 0, yaw))))
...
def make_drone_group(p: MassDriverParams) -> bd.Compound:
    pal = _palette(p)
    body = _tinted(bd.Box(950, 950, 480), "drone_body", COL_WHITE)
    cam = _tinted(bd.Sphere(240).moved(bd.Location((0, 0, -380))), "drone_camera_ball", COL_GLASS)
    parts = [body, cam]
    for i, (ax, ay) in enumerate(((1, 1), (1, -1), (-1, 1), (-1, -1))):
        arm = bd.Box(900, 160, 120).moved(bd.Location((0, 0, 0), (0, 0, degrees(atan2(ay, ax)))))
        parts.append(_inst(arm, f"drone_arm_{i + 1}", bd.Location((ax * 620, ay * 620, 120))))
        parts[-1].color = COL_WHITE_DIM
        parts.append(_tinted(bd.Cylinder(420, 60).moved(bd.Location((ax * 1_050, ay * 1_050, 220))),
                             f"drone_rotor_{i + 1}", COL_STEEL_DARK))
    drone = _grp("inspection_drone", parts)
    drones = []
    for k, (x, y, z) in enumerate(DRONE_HOMES):
        drones.append(_inst(drone, f"inspection_drone_{k + 1:02d}", bd.Location((x, y, z))))
...
def make_dust_group(p: MassDriverParams) -> bd.Compound:
    ...
    for label, x, y, r, h in DUST_PLUMES:
        plume = bd.Cone(220, r * scale, h * scale).moved(bd.Location((x, y, h * scale / 2)))
        parts.append(_tinted(plume, label, bd.Color(1.0, 0.44, 0.14, alpha)))
    for k, (x, y, _yaw) in enumerate(rover_home_poses(p)):
        plume = bd.Cone(120, 1_100 * scale, 1_500 * scale).moved(
            bd.Location((x - 2_900, y, 1_500 * scale / 2)))
        parts.append(_tinted(plume, f"rover_dust_plume_{k + 1:02d}", bd.Color(1.0, 0.44, 0.14, alpha)))
...
def make_terrain_group(p: MassDriverParams) -> bd.Compound:
    pal = _palette(p)
    parts = [
        _tinted(bd.Box(228_000, 96_000, 4_000).moved(bd.Location((86_000, 0, -2_000))),
                "lunar_terrain_slab", COL_TERRAIN),
        _tinted(bd.Box(210_000, 3_600, 120).moved(bd.Location((80_000, ROAD_Y, 60))),
                "service_road", COL_TERRAIN_DARK),
        _tinted(bd.Cylinder(14_000, 250).moved(bd.Location((PAD_CENTER[0], PAD_CENTER[1], 125))),
                "landing_pad", COL_PAD),
        _tinted(bd.Torus(12_500, 90).moved(bd.Location((PAD_CENTER[0], PAD_CENTER[1], 260))),
                "landing_pad_marking_ring", _rgba(pal["marking"])),
    ]
    for k, (x, y, r) in enumerate(CRATERS):
        parts.append(_tinted(bd.Torus(r, r * 0.09).moved(bd.Location((x, y, 40))),
                             f"crater_rim_{k + 1}", COL_TERRAIN_DARK))
        parts.append(_tinted(bd.Cylinder(r * 0.9, 70).moved(bd.Location((x, y, 20))),
                             f"crater_floor_{k + 1}", COL_TERRAIN_DARK))
    for k, (x, y, r) in enumerate(BOULDERS):
        parts.append(_tinted(bd.Sphere(r).moved(bd.Location((x, y, r * 0.35))),
                             f"boulder_{k + 1}", COL_TERRAIN_DARK))
    return _grp("lunar_terrain", parts)


def make_earth_group(p: MassDriverParams) -> bd.Compound:
    if not p.earth_visibility:
        stub = _tinted(bd.Sphere(200).moved(bd.Location(EARTH_POS)), "earth_omitted_marker", COL_EARTH)
    ...
    band = bd.Torus(6_700, 450)
    band_a = _inst(band, "earth_cloud_band_north",
                   bd.Location(EARTH_POS) * bd.Location((0, 0, 2_050), (12, 0, 0)))
    band_a.color = COL_EARTH_HAZE
    band_b = _inst(band, "earth_cloud_band_south",
                   bd.Location(EARTH_POS) * bd.Location((0, 0, -2_050), (-8, 0, 0)))
    band_b.color = COL_EARTH_HAZE
    parts = [
        _tinted(bd.Sphere(7_000).moved(bd.Location(EARTH_POS)), "earth_globe", COL_EARTH),
        band_a,
        band_b,
    ]
...
def build_scene(p: MassDriverParams = DEFAULTS) -> bd.Compound:
    ...
    if dist > 0.0:
        ...
        for key, group in zip(keys, groups):
            ...
            moved = group.moved(bd.Location((dx * dist, dy * dist, dz * dist)))
...
@step(out="../STEP/lunar_mass_driver.step")
def lunar_mass_driver():
