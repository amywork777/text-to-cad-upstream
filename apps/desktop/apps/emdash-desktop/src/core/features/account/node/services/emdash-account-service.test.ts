...
import { AccountService } from './emdash-account-service';
...
vi.mock('../config', () => ({
  ACCOUNT_CONFIG: {
    authServer: { baseUrl: 'https://auth.hardcore.test', authTimeoutMs: 5000 },
  },
}));
...
describe('AccountService', () => {
  ...
  describe('linkProviderAccount()', () => {
    it('starts an account-link transaction and stores the linked provider token', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://auth.hardcore.test/api/v1/auth/electron/account-link/start',
      ...
      expect(mockExecuteOAuthFlow).toHaveBeenCalledWith({
        authorizeUrl: 'https://auth.hardcore.test/api/v1/auth/electron/account-link/authorize',
        exchangeUrl: 'https://auth.hardcore.test/api/v1/auth/electron/exchange',
        successRedirectUrl: 'https://auth.hardcore.test/auth/success',
        errorRedirectUrl: 'https://auth.hardcore.test/auth/error',
        extraParams: {
    ...
    it('clears stale local session when account-link start reports no active session', async () => {
      ...
      expect(error).toMatchObject({
        ...
        message: 'Your app session expired. Connect GitHub again.',
      });
  ...
  describe('checkServerHealth()', () => {
    it('returns true on successful health check', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        'https://auth.hardcore.test/health',
