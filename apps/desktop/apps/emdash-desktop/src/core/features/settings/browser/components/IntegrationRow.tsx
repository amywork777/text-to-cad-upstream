  ...
  return (
    <div className="group hover:bg-muted/40 relative grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg px-3 py-2 transition-colors">
      ...
      <div className="text-muted-foreground flex items-center justify-end gap-2 text-sm">
        ...
        {showStatusPill ? (
          <span
            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-micro font-medium tracking-[1.25px] uppercase ${STATUS_CLASSES[resolvedStatus] ?? STATUS_CLASSES.disconnected}`}
          >
            {resolvedStatusLabel}
          </span>
        ) : null}
