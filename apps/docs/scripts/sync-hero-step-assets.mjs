...
//
//   cadgen step build models/step/assemblies/planetary_gear_assembly.step
...
const modelStep = path.join(repoRoot, "models/step/assemblies/planetary_gear_assembly.step");
...
if (!fs.existsSync(path.join(packageDir, "assembly.json"))) {
  throw new Error(
    `No render package for ${modelStep} at ${packageDir} — run: cadgen step build ${modelStep}`,
  );
}
