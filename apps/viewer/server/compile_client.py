...
CADGEN_UNAVAILABLE = (
    "importing a STEP file requires cadgen, which is not installed in the Python "
    "running this Viewer. Install it (pip install -r requirements.txt beside the "
    "Viewer, or pip install cadgen). Viewing existing models does not need cadgen."
)
...
def set_cadgen_probe_for_tests(probe) -> None:
    """Force the availability answer.

    The JS backend had ``_setCadgenProbeForTests`` for exactly this, and two
    suites depend on being able to say "no cadgen here" without uninstalling it,
    or "yes" without paying a spawn. ``None`` restores the real probe.
    """
...
class CompileClient:
    ...
    def _worker_argv(self, port: int, token: str) -> list[str]:
        ...
        worker = os.path.join(os.path.dirname(os.path.abspath(__file__)), "compile_worker.py")
        return [sys.executable, worker, "--frame-port", str(port), "--token", token]
