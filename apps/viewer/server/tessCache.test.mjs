...
import {
  readCachedTessellationBytes as cadgen_jsRead,
  tessellationCacheDir as cadgen_jsCacheDir,
  writeCachedTessellationBytes as cadgen_jsWrite,
} from "../packages/cadgen-js/src/lib/surf/tessellationCacheFs.mjs";
...
test("the inlined store IS the cadgen-js store: same dir, interoperable bytes", async (t) => {
  // server/tessCache.mjs carries an inline copy of tessellationCacheFs.mjs
  // because the bundled skill runtime has no cadgen-js tree. This is the drift
  // fence: same resolved directory, and entries written by either side read
  // back through the other.
  const home = sandboxHome(t);
  assert.equal(tessellationCacheDir(), cadgen_jsCacheDir());
  assert.ok(tessellationCacheDir().startsWith(home));
  cadgen_jsWrite(KEY, Buffer.from("FROM-CADJS"));
  const viaServer = await fetchThroughApp(t, KEY);
  assert.equal(viaServer, "FROM-CADJS");
  assert.equal(Buffer.from(cadgen_jsRead(KEY)).toString(), "FROM-CADJS");
});
...
test("batch route speaks the cadgen-js TESB codec: hits, misses, refusals", async (t) => {
  const home = sandboxHome(t);
  const base = await startApp(t);
  ...
  assert.ok(entries, "server framing must decode with the cadgen-js codec");
