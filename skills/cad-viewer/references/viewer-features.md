...
- File sheet: open the right sheet for file-specific tabs. STEP files get Tree, Reference, and Measure, plus Kinematics and Animation tabs when the model's sidecar declares kinematics or animation clips, and Display. In Kinematics, a DOF that exactly one coupling gears is marked "driven by <coupling>": its slider shows the effective value and drags the coupling, so sliding one member of a gear train turns the whole train. URDF/SRDF/SDF files get joints and metadata. Mesh files show a Measure tab for vertex-to-vertex distance. DXF drawings have material/bend controls.
