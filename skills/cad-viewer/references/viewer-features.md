...
- File names read exactly as they do on disk. The tab title, breadcrumb, catalog rows and the file picker all show the artifact's own basename and its own path — `moonwatch.step`, never the `moonwatch.py` that generated it. Generated-ness still shows through status badges and rebuild behaviour, and the sidecar still records provenance for freshness gates, but no part of the UI names or opens a model script. (Copied topology references are the one exception, and a deliberate one: see the bare-stem rule below.)
...
- File sheet: open the right sheet for file-specific tabs. STEP files get Tree, Reference, and Measure, plus Kinematics and Animation tabs when the model's sidecar declares kinematics or animation clips, and Display. In Kinematics, a DOF that exactly one coupling gears is marked "driven by <coupling>": its slider shows the effective value and drags the coupling, so sliding one member of a gear train turns the whole train. In Animation, the "Clip" dropdown leads with a built-in "No clip" entry — the transport's idle state, in which the model holds the Kinematics tab's pose — followed by the model's authored clips under a "Clips" heading; a pose or clip named `rest` is never confused with it, and Play with no clip selected starts the first authored clip. URDF/SRDF/SDF files get joints and metadata. Mesh files show a Measure tab for vertex-to-vertex distance. DXF drawings have material/bend controls.
