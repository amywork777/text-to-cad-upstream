...
reports `needs-build` and the in-Viewer build spawns `cadgen step build` — the one
...
agent is doing the work, running `cadgen step build <file>` directly is equivalent
