...
8. Run `cadgen implicit gen <model.implicit.js>` to build (or refresh) the render package the CAD Viewer opens. Add `--write` when a sibling `<name>.glb` is also wanted, or use `cadgen implicit export --input <model.implicit.js> --glb` for STL/3MF, non-default parameters, or an animation.
...
```bash
cadgen implicit snapshot --input models/implicit-cad/<model>.implicit.js --output /tmp/implicit-review/<model>.png
```
...
```bash
cadgen implicit snapshot --job - <<'JSON'
{
...
```bash
cadgen implicit gen <model.implicit.js>
cadgen implicit gen <model.implicit.js> --write
cadgen implicit gen models/implicits/*.implicit.js --force
cadgen implicit gen <model.implicit.js> --resolution 128 --threads 4
cadgen implicit gen --help
```

`cadgen implicit gen` builds the model's **render package** — the baked mesh the CAD Viewer opens,
...
substitute). Use `cadgen implicit export` when you want STL or 3MF, non-default parameters, or an
animation.
...
```bash
cadgen implicit export --input <model.implicit.js> --glb
cadgen implicit export --input <model.implicit.js> --stl <mesh.stl> --resolution <resolution>
cadgen implicit export --input <model.implicit.js> --stl --3mf --glb
cadgen implicit export --input <model.implicit.js> --3mf --params '<parameter-json>' --json
cadgen implicit export --help
```

One flag per format — `--stl`, `--3mf`, `--glb` — and several are allowed in one run, matching the CAD skill's export CLI. At least one is required; without any, the command exits 2. The model is meshed once per run, so every requested format comes from identical geometry. A format flag with no path writes next to the source file using the same stem, such as `<model>.glb` for `<model>.implicit.js`; a supplied path resolves against the current directory. Use `cadgen implicit export --help` for the complete current command interface.
